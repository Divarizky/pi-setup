...
[Prasyarat](../shared/COMMON.md#prasyarat) — deteksi mode sesuai `../WORKFLOW.md`. Dalam Universal mode, lanjut tanpa membuat artifact workflow; catat mode dan keterbatasan context di respons bila relevan.
...
- Migrasi project → `setup-workflow` lalu `project-migration` (wajib project-aware)
...
Gunakan Context Resolver. Baca `AGENT.md`, `CONTEXT.md`, dan `ADR.md` hanya jika tersedia. Dalam Universal mode, gunakan context percakapan dan file project relevan; jangan membuat atau memperbarui context artifact. Tanya satu per satu, skip yang jelas:
...
Istilah baru → Project-aware mode: update file inline mengikuti **Aturan Split** di `setup-workflow`. Universal mode: tampilkan definisi/keputusan di chat dan catat statusnya di respons; jangan menulis context artifact. Bentrok → klarifikasi sinonim.
...
- Project-aware mode: output ke AGENT.md (quick) + CONTEXT.md (detail) + ADR pertama — ikuti **Aturan Split**
- Universal mode: tampilkan hasil domain model di chat dan catat statusnya di respons; jangan membuat file

Aturan kedua mode: eksplorasi codebase dulu kalau bisa jawab. Tulis ke AGENT.md/CONTEXT.md hanya dalam Project-aware mode; Universal mode tetap chat-only.
...
**Multi-Agent**: jika `.workspace/project-meta.md` tersedia, gunakan state Project-aware yang sudah dipersist. Tanpa marker, lanjut Universal mode dan jangan menganggap context tersedia di agent/sesi berikutnya.
...
- Cek mode sebelum route. Tawarkan `setup-workflow` hanya untuk persistence lintas sesi atau route yang memang wajib Project-aware.
