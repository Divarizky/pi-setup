...
| `docs/rpc.md` | Calling this extension from another pi extension — `pi.events` lifecycle events (`subagents:completed`, `subagents:ready`, …), the `subagents:rpc:*` channels (`ping`, `spawn`, `stop`, `consume`), spawn options, error strings, and the `Symbol.for("subagents:manager")` registry. Source: `src/cross-extension-rpc.ts`. |
...
- Attribute external contributions with a link to the source change when this
  repository is later shared publicly.

## Releasing

**Versioning**: this private extension is currently on the `2.x` line. Breaking
changes use a major version; compatible features use a minor version; fixes use
an incremented patch version.
