...
function loadFromDir(dir: string, agents: Map<string, AgentConfig>, source: "project" | "global", strict: boolean): void {
  ...
  for (const file of files) {
    ...
    agents.set(name, {
      ...
      taskPolicy: parseTaskPolicy(fm.task_policy),
...
function parseTaskPolicy(val: unknown): "scout" | "ship" | "flexible" | undefined {
  if (val === "scout" || val === "ship" || val === "flexible") return val;
  return undefined;
}
