...
export function createLeadJobId(title: string, suffix?: string) {
  const leadTitle = title.replace(/[\s_-]+lead$/i, "").trim();
  return createJobId(`lead-${leadTitle}${suffix ? `-${suffix}` : ""}`);
}
...
export function createBranchName(
  ...
  const scope = rawScope
    ? safeSegment(rawScope, "scope").slice(0, 32)
    : undefined;
...
export async function removeSubagentWorktree(
  ...
  const repoRoot = await realpath(worktree.repoRoot).catch(() => {
    throw new WorktreeError(
      `Repository root is not resolvable: ${worktree.repoRoot}`,
    );
  });
  ...
  if (!targetPath) {
    if (!options.deleteBranch)
      throw new WorktreeError(
        `Refusing to retire unverified worktree: ${normalizePath(worktree.path)}`,
      );
  ...
  try {
    currentRoot = (
      await runGit(["rev-parse", "--show-toplevel"], targetPath)
    ).stdout.trim();
    currentBranch = (
      await runGit(["branch", "--show-current"], targetPath)
    ).stdout.trim();
    const currentCommon = (
      await runGit(["rev-parse", "--git-common-dir"], targetPath)
    ).stdout.trim();
    const repoCommon = (
      await runGit(["rev-parse", "--git-common-dir"], repoRoot)
    ).stdout.trim();
    currentCommonDir = path.isAbsolute(currentCommon)
      ? currentCommon
      : path.resolve(targetPath, currentCommon);
    repoCommonDir = path.isAbsolute(repoCommon)
      ? repoCommon
      : path.resolve(repoRoot, repoCommon);
  } catch {
    throw new WorktreeError(
      `Refusing to retire unverified worktree: ${target}`,
    );
  }
  if (
...
export async function assertBranchAvailable(
  ...
  if (refs.some((ref) => ref === branch || ref.endsWith(`/${branch}`))) {
    throw new WorktreeError(
      `Branch already exists; refusing collision: ${branch}`,
    );
  }
}
...
export async function createSubagentWorktree(options: {
  ...
  const branch = options.branchName ?? `chore/${jobId}`;
