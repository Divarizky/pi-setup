...
import { BACKEND_NAMES, type BackendName, type SubagentMode, type SubagentStatus } from "./domain.ts"
...
interface StateFile {
  ...
  readonly jobs: ReadonlyArray<PersistedJob>
}
...
function parseJob(value: unknown): PersistedJob {
  ...
  if (value.status !== "running" && value.status !== "done" && value.status !== "error") {
  ...
  if (value.backend !== undefined && !BACKEND_NAMES.includes(value.backend as BackendName)) {
  ...
  const sessionFilePath = typeof value.sessionFilePath === "string"
    ? value.sessionFilePath.slice(0, 4_096)
    : undefined
  const nativeSessionId = typeof value.nativeSessionId === "string"
    ? value.nativeSessionId.slice(0, 512)
    : undefined
  const nativeTerminalHandle = typeof value.nativeTerminalHandle === "string"
    ? value.nativeTerminalHandle.slice(0, 512)
    : undefined
  const nativeWorktreeId = typeof value.nativeWorktreeId === "string"
    ? value.nativeWorktreeId.slice(0, 512)
    : undefined
  const nativeTabId = typeof value.nativeTabId === "string"
    ? value.nativeTabId.slice(0, 512)
    : undefined
  const nativePaneKey = typeof value.nativePaneKey === "string"
    ? value.nativePaneKey.slice(0, 512)
    : undefined
  const nativeLaunchToken = typeof value.nativeLaunchToken === "string"
    ? value.nativeLaunchToken.slice(0, 512)
    : undefined
  const title = value.title as string
  const cwd = value.cwd as string
  const settledAt = typeof value.settledAt === "number" ? value.settledAt : undefined
  ...
  const errorText = typeof value.errorText === "string"
...
export class JobPersistence {
  ...
  async loadApprovals(): Promise<ReadonlyArray<ApprovalRequest>> {
    ...
    try {
      ...
      throw new PersistenceError(`Cannot read durable approval state: ${String(error)}`)
    }
    let parsed: unknown
    try {
      parsed = JSON.parse(raw)
    } catch {
      throw new PersistenceError("Durable approval state is malformed JSON; refusing to overwrite it.")
    }
    if (!isRecord(parsed) || parsed.version !== VERSION || !Array.isArray(parsed.approvals)) {
      throw new PersistenceError("Durable approval state has an unsupported schema.")
    }
    const seen = new Set<string>()
    return parsed.approvals.map((value) => {
      if (!isRecord(value)) throw new PersistenceError("Malformed persisted approval.")
      const strings = ["id", "jobId", "operation", "status"]
      for (const key of strings) {
        if (typeof value[key] !== "string" || value[key].length === 0) {
          throw new PersistenceError(`Malformed persisted approval field: ${key}.`)
        }
      }
      if (!["review", "commit", "merge", "push", "pr", "delete-worktree"].includes(value.operation as string)) {
        throw new PersistenceError("Malformed persisted approval operation.")
      }
      if (!["pending", "approved", "executing", "rejected", "consumed"].includes(value.status as string)) {
        throw new PersistenceError("Malformed persisted approval status.")
      }
      const id = value.id as string
      if (seen.has(id)) throw new PersistenceError(`Duplicate persisted approval: ${id}`)
      seen.add(id)
      ...
      if (value.status !== "pending" && (value.decidedBy !== "human" || typeof value.decidedAt !== "number" || !Number.isFinite(value.decidedAt))) {
        throw new PersistenceError("Persisted approval lacks verified human decision evidence.")
      }
      return {
        id,
        ...
        ...(typeof value.decidedAt === "number" ? { decidedAt: value.decidedAt } : {}),
        ...(value.decidedBy === "human" ? { decidedBy: "human" as const } : {}),
        ...(typeof value.reason === "string" ? { reason: value.reason.slice(0, MAX_ERROR_BYTES) } : {}),
      }
    })
  }

  async saveApprovals(approvals: ReadonlyArray<ApprovalRequest>): Promise<void> {
  ...
  async load(): Promise<ReadonlyArray<PersistedJob>> {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === "ENOENT") return []
      throw new PersistenceError(`Cannot read durable job state: ${String(error)}`)
    }
    let parsed: unknown
    try {
      parsed = JSON.parse(raw)
    } catch {
      throw new PersistenceError("Durable job state is malformed JSON; refusing to overwrite it.")
    }
    if (!isRecord(parsed) || parsed.version !== VERSION || !Array.isArray(parsed.jobs)) {
      throw new PersistenceError("Durable job state has an unsupported schema.")
    }
  ...
  async loadEvents(): Promise<ReadonlyArray<JobEvent>> {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === "ENOENT") return []
      throw new PersistenceError(`Cannot read durable job events: ${String(error)}`)
    }
    return raw
      .split(/\r?\n/)
      ...
      .map((line) => {
        ...
        try {
          ...
          throw new PersistenceError("Durable job event log is malformed JSON; refusing replay.")
        }
        if (!isRecord(value) || typeof value.jobId !== "string" || typeof value.event !== "string" || typeof value.at !== "number") {
          throw new PersistenceError("Durable job event has an invalid schema; refusing replay.")
        }
        return {
          at: value.at,
          jobId: value.jobId,
          event: value.event,
          ...(typeof value.message === "string" ? { message: value.message.slice(0, MAX_ERROR_BYTES) } : {}),
        }
      })
  }
