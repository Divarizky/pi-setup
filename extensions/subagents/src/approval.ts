...
import type { SubagentJobId, SubagentMode } from "./domain.ts";
...
export const DELIVERY_PREREQUISITES: Readonly<
  Record<ApprovalOperation, ReadonlyArray<ApprovalOperation>>
> = {
  review: [],
  commit: ["review"],
  merge: ["commit"],
...
export type ApprovalStatus =
  "pending" | "approved" | "executing" | "rejected" | "consumed";
...
function approvalId(jobId: SubagentJobId, operation: ApprovalOperation) {
...
export class ApprovalGate {
  ...
  request(options: {
    ...
    if (options.mode !== "build") {
      throw new ApprovalGateError(
        "Only build subagents may request delivery approval.",
      );
    }
  ...
  ): ReadonlyArray<ApprovalOperation> {
    return DELIVERY_PREREQUISITES[operation].filter(
      (required) =>
  ...
  restore(requests: ReadonlyArray<ApprovalRequest>) {
    for (const request of requests) {
      ...
      if (this.requests.has(request.id)) {
        throw new ApprovalGateError(
          `Duplicate approval request: ${request.id}`,
        );
      }
  ...
  reject(id: string, reason: string, now = Date.now()): ApprovalRequest {
    if (!reason.trim())
      throw new ApprovalGateError("Rejection reason is required.");
  ...
  complete(id: string): ApprovalRequest {
    ...
    if (!current)
      throw new ApprovalGateError(`Unknown approval request: ${id}`);
    if (current.status !== "executing")
      throw new ApprovalGateError(
        `Approval ${id} is ${current.status}; execution was not started.`,
      );
  ...
  private decide(id: string, status: "approved", now: number) {
    ...
    if (missing.length > 0) {
      throw new ApprovalGateError(
        `Approval ${id} is missing consumed prerequisite(s): ${missing.join(", ")}.`,
      );
    }
  ...
  private requireApproved(id: string) {
    ...
    if (current.status !== "approved") {
      throw new ApprovalGateError(
        `Approval ${id} is ${current.status}; explicit approval is required before delivery.`,
      );
    }
    return current;
  }
