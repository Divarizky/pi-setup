  ...
  Effect.gen(function* () {
    ...
    const runTurn = (userText: string, turn: number) =>
      Effect.gen(function* () {
        ...
        yield* emit({ _tag: "ToolStart", toolId, name: profile.toolName, argsPreview })
        yield* pause
        yield* emit({
          _tag: "ToolUpdate",
          toolId,
        ...
        yield* emit({ _tag: "RunSettled", outcome: { _tag: "Completed", finalText } })
      })
      ...
      while (true) {
        ...
        const fiber = yield* Effect.forkChild(
          runTurn(text, turn).pipe(
            Effect.onInterrupt(() =>
              emit({ _tag: "RunSettled", outcome: { _tag: "Interrupted" } }).pipe(Effect.ignore),
            ),
          ),
        )
    ...
    const submit = (text: string) =>
      Effect.gen(function* () {
        if (state.closed) {
          return yield* new SendError({ message: "Subagent session is closed." })
        }
    ...
    yield* Effect.try(() => fs.mkdirSync(STUB_DIR, { recursive: true })).pipe(Effect.ignore)
