...
export function excludedToolsForMode(mode: SubagentMode = "build") {
  return mode === "scout"
...
async function createChildResources(
  ...
  if (parentModel) {
    const providerConfig = parentRegistry.getRegisteredProviderConfig(parentModel.provider)
    if (providerConfig) modelRuntime.registerProvider(parentModel.provider, providerConfig)
  }
  const model = parentModel
    ? modelRuntime.getModel(parentModel.provider, parentModel.id) ?? parentModel
    : undefined
...
function assistantParts(msg: AssistantMessage): TranscriptPart[] {
  ...
  for (const part of msg.content) {
    if (part.type === "text") {
      ...
      parts.push({ type: "thinking", text: part.redacted ? "" : part.thinking ?? "", redacted: part.redacted })
    } else if (part.type === "toolCall") {
      parts.push({
        type: "toolCall",
        toolId: part.id ?? "",
        name: part.name ?? "",
        argsPreview: safeJson((part as unknown as { arguments?: unknown }).arguments),
      })
    }
  }
  return parts
...
const makePiSession = (
  ...
  Effect.gen(function* () {
    ...
    const thinkingLevel = task.reasoningEffort ?? task.parent.inheritedThinkingLevel

    const session = yield* Effect.tryPromise({
      try: async () => {
        const { loader, settingsManager, modelRuntime, model: childModel } = await createChildResources(
          task.cwd,
    ...
    const activeModel = (): Model<any> | undefined => {
      ...
      return (
        registry.find(last.provider, last.responseModel ?? last.model) ?? sessionModel
      )
    }
    ...
    const settle = () => {
      ...
      if (last?.stopReason === "aborted") {
        emit({ _tag: "RunSettled", outcome: { _tag: "Interrupted", partialText } })
        return
      }
      const errorText =
        state.runError ??
      ...
      if (errorText !== undefined) {
        emit({
          _tag: "RunSettled",
          outcome: { _tag: "Failed", errorText: boundedError(errorText), partialText },
        })
        return
      }
      emit({ _tag: "RunSettled", outcome: { _tag: "Completed", finalText: finalOutput(session) } })
    }
      ...
      switch (event.type) {
        ...
        case "message_update": {
          ...
          if (streamEvent?.type === "text_delta") {
            emit({ _tag: "AssistantDelta", kind: "text", delta: streamEvent.delta ?? "" })
          } else if (streamEvent?.type === "thinking_delta") {
            emit({ _tag: "AssistantDelta", kind: "thinking", delta: streamEvent.delta ?? "" })
          }
          break
        }
        case "message_end": {
        ...
        case "queue_update": {
          ...
          state.queuedPromptBytes = queued.reduce((total, item) => total + Buffer.byteLength(item.text, "utf8"), 0)
    ...
    emit({ _tag: "MetaChanged", meta: currentMeta() })
    startRun(buildSubagentExecutionPrompt({
      ...
      send: (text) =>
        Effect.suspend((): Effect.Effect<void, SendError> => {
          ...
          if (text.length > 32_000) {
            return new SendError({ message: "Subagent prompt exceeds the 32,000-character limit." })
          }
          if (session.isStreaming) {
            const bytes = Buffer.byteLength(text, "utf8")
            if (state.queuedPromptCount >= MAX_QUEUED_PROMPTS || state.queuedPromptBytes + bytes > MAX_QUEUED_PROMPT_BYTES) {
              return new SendError({ message: "Subagent prompt queue is full." })
            }
