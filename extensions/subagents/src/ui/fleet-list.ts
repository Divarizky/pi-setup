 ...
 *
 * Mechanics: the list is rendered inside a chained footer, directly above the
 * custom footer. ALL key handling goes through `onTerminalInput` — which fires
 * before the focused editor and can `consume` keys — gated on
 * `getEditorText() === ""` so normal typing is untouched.
 */

import { Editor, isKeyRelease, Key, matchesKey, truncateToWidth, visibleWidth } from "@earendil-works/pi-tui";
import { hasAgentBadge, renderAgentName } from "../agent-color.js";
import { type AgentManager, isTopLevelAgent } from "../core/agent-manager.js";
...
import { mountChainedFooter, readChainedFooter } from "./footer-chain.js";
...
export type FleetUICtx = {
  setFooter(
    factory: ((tui: any, theme: Theme, footerData: any) => { render(width: number): string[]; invalidate(): void; dispose?(): void }) | undefined,
  ): void;
...
export class FleetList {
  ...
  /** Restore factory custom saat composite dilepas. */
  private restoreFooter: (() => void) | undefined;
  /** Factory custom yang sedang dibungkus — re-chain bila publish berubah. */
  private chainedFactory: unknown = undefined;
  ...
  ensureTimer(): void {
    // Guarded: with no UI attached there is nothing to render, and the timer
    // callback would throw on `this.ui.setFooter` in wiring tests whose mock
    // ctx carries no setFooter. The tool handler calls update() directly, so
    // skipping the tick here loses nothing.
    if (!this.ui) return;
  ...
  dispose(): void {
    ...
    if (this.ui && this.widgetRegistered) this.unmountComposite();
    this.widgetRegistered = false;
    this.tui = undefined;
    this.active = false;
  ...
  /** Footer-chain memasok TUI aktif untuk refresh dan deteksi focus. */
  setTui(tui: any): void { this.tui = tui; }

  /** Render baris fleet — dipakai composite footer-chain. */
  renderFleetLines(width: number, theme: Theme): string[] {
    return this.renderBar(width, theme);
  }

  /** True saat user masuk manual ke list (navigasi aktif) — pemisah tampil. */
  isActive(): boolean {
    return this.active;
  }

  /** Lepas composite, kembalikan factory custom asli. */
  private unmountComposite(): void {
    try { this.restoreFooter?.(); } finally {
      this.restoreFooter = undefined;
      this.chainedFactory = undefined;
      this.widgetRegistered = false;
      this.tui = undefined;
    }
  }

  /** Re-chain composite (baca ulang factory custom — urutan load aman). */
  private mountComposite(): void {
    // Lepas dulu agar instance lama di-dispose pi sebelum pasang baru.
    this.restoreFooter?.();
    const ui = this.ui!;
    this.chainedFactory = readChainedFooter();
    this.restoreFooter = mountChainedFooter((f) => ui.setFooter(f), this);
    this.widgetRegistered = true;
  }

  /** Re-chain composite bila factory custom berubah; refresh bila sudah terpasang. */
  update(): void {
    if (!this.ui) return;
    ...
    if (!hasRows) {
      if (this.widgetRegistered) this.unmountComposite();
      if (this.timer) { clearInterval(this.timer); this.timer = undefined; }
      this.active = false;
    ...
    // Chain ulang hanya bila factory custom berubah (mis. pemilik load
    // belakangan / session baru) — bukan tiap tick, agar tidak churn render.
    if (!this.widgetRegistered || readChainedFooter() !== this.chainedFactory) {
      this.mountComposite();
    } else {
  ...
  private renderBar(width: number, theme: Theme): string[] {
    ...
    const lines: string[] = [];
    // Rata kiri dengan footer custom (tanpa indent) + garis pemisah saat aktif.
    if (this.active) lines.push(truncateToWidth(theme.fg("border", "─".repeat(Math.max(0, width))), width));
