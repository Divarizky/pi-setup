...
function statusGlyph(snap: SubagentSnapshot, theme: Theme): string {
  ...
  if (isSubagentBooting(snap)) return theme.fg("warning", "BOOT");
  switch (snap.status) {
    case "running":
...
function compactPath(value: string, width: number): string {
  return truncateToWidth(value, Math.max(12, width));
}

function latestInboxError(snap: SubagentSnapshot): string | undefined {
  return [...snap.eventLog]
    .reverse()
    .find(
      (event) =>
        event.event === "BackendError" &&
        event.message?.startsWith("Orca steering inbox:"),
    )?.message;
}

export function formatSubagentStats(snap: SubagentSnapshot): string {
  const report = snap.report;
  const passed = report?.tests.filter((test) => test.passed).length ?? 0;
  const testLabel =
    report && report.tests.length > 0
      ? `Results: ${passed}/${report.tests.length} passed`
      : "Results: not reported";
  const changes = report?.changes.length ?? 0;
  const queue =
    snap.queued.length > 0
      ? snap.backend === "orca"
        ? `Orca inbox: ${snap.queued.length}`
        : `Queue: ${snap.queued.length}`
      : "Queue: empty";
  const issues =
    snap.metrics.restartCount > 0 || snap.metrics.timeoutCount > 0
      ? ` · Restarts: ${snap.metrics.restartCount} · Timeouts: ${snap.metrics.timeoutCount}`
      : "";
  return `${testLabel} · Changes: ${changes} · Turns: ${snap.turns} · Runs: ${snap.metrics.runCount}${issues} · ${queue}`;
}

export function formatSubagentStatsLines(
  snap: SubagentSnapshot,
  width: number,
  theme: Theme,
): string[] {
  return wrapTextWithAnsi(formatSubagentStats(snap), Math.max(12, width)).map(
...
function statusWord(snap: SubagentSnapshot, theme: Theme): string {
  ...
  if (isSubagentBooting(snap)) return theme.fg("warning", "booting");
  switch (snap.status) {
    case "running":
...
export interface TakeoverOptions {
  ...
  readonly onApprove?: (jobId: string, approvalId: string) => Promise<void>;
  readonly onConfirmAction?: (actionId: string) => Promise<void>;
  readonly onInspectTerminal?: (snap: SubagentSnapshot) => Promise<void>;
}
...
function labeledLines(
  ...
  return wrapTextWithAnsi(`${label}${value}`, Math.max(12, width)).slice(
    0,
    maxLines,
  );
}
...
export function buildSubagentInfoLines(
  ...
  const details = [
    ...
    snap.queued.length > 0
      ? snap.backend === "orca"
        ? `${snap.queued.length} durable queued`
        : `${snap.queued.length} queued`
      : undefined,
    approvals.length > 0
    ...
    actions.length > 0
      ...
      : undefined,
    snap.report?.needsParentDecision ? "DECISION NEEDED" : undefined,
  ...
  if (report) {
    ...
    lines.push(
      theme.fg(
        report.needsParentDecision ? "warning" : "dim",
        truncateToWidth(reportLine, width),
      ),
    );
    lines.push(
      ...labeledLines("summary: ", sanitizeText(report.summary), width, 2).map(
  ...
  if (approvals.length > 0) {
    lines.push(
      theme.fg(
        "warning",
        truncateToWidth(
          `pending approval: ${approvals.map((item) => item.operation).join(", ")}`,
          width,
        ),
      ),
    );
  ...
  if (actions.length > 0) {
    lines.push(
      theme.fg(
        "warning",
        truncateToWidth(
          `pending action: ${actions.map((item) => item.event.type).join(", ")}`,
          width,
        ),
      ),
    );
  }
  const inboxError = latestInboxError(snap);
  if (inboxError)
    lines.push(theme.fg("error", truncateToWidth(inboxError, width)));
  return lines;
}
...
export function confirmSubagentDeletion(
  ...
  return ctx.ui.custom<boolean>(
    (_tui, theme, keybindings, done) =>
      new DeleteConfirmationView(theme, keybindings, snap, done),
    {
      overlay: true,
      overlayOptions: {
        anchor: "center",
...
class DeleteConfirmationView implements Component {
  ...
  render(width: number): string[] {
    ...
    const messageLines = wrapTextWithAnsi(
      `Delete "${this.snap.title}" and its session/chat history?`,
      innerWidth,
    );
    const content = [
      title,
      ...
      this.theme.fg(
        "warning",
        "The session, durable metadata, worktree, branch, and all uncommitted changes will be deleted.",
      ),
      "",
    ...
    const framed = content.map(
      (line) =>
        ...
        this.pad(line, innerWidth) +
        this.theme.fg("border", " │"),
    );
    return [top, ...framed, bottom].map((line) =>
      truncateToWidth(line, frameWidth),
    );
  }
...
class SubagentDashboard implements Component {
  ...
  handleInput(data: string): void {
    ...
    if (data === "r") {
      const snap = subs[this.selection.index];
      if (snap?.status === "failed" && this.options?.onRetry) {
        this.runAction(
          () => this.options!.onRetry!(snap.id),
          "Retry requested",
        );
      }
      return;
    ...
    if (data === "a") {
      ...
      if (approval && this.options?.onApprove) {
        this.runAction(
          () => this.options!.onApprove!(snap!.id, approval.id),
          "Approval processed",
        );
      }
      return;
    ...
    if (data === "c") {
      ...
      const action =
        ...
        this.options
          ?.getActions?.(snap.id)
          .find((item) => item.status === "pending");
      if (action && this.options?.onConfirmAction) {
        this.runAction(
          () => this.options!.onConfirmAction!(action.event.actionId),
          "Action confirmed",
        );
      }
      return;
    ...
    if (data === "i") {
      const snap = subs[this.selection.index];
      if (snap?.backend === "orca" && this.options?.onInspectTerminal) {
        this.runAction(() => this.options!.onInspectTerminal!(snap));
      }
      return;
    }
    if (data === "d") {
      const snap = subs[this.selection.index];
      if (snap && this.options?.onDelete && !this.deleting)
        void this.deleteThread(snap);
      return;
    }
    if (data === "h") {
  ...
  private runAction(action: () => Promise<void>, successMessage?: string) {
    this.actionError = undefined;
    void action()
      .then(() => {
        if (successMessage) this.actionError = successMessage;
        this.tui.requestRender();
      })
      .catch((error: unknown) => {
        this.actionError =
  ...
  private async deleteThread(snap: SubagentSnapshot): Promise<void> {
    ...
    try {
      ...
      this.options?.onDeleteError?.(
        `Could not delete ${snap.id}: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      this.deleting = false;
      this.tui.requestRender();
  ...
  render(width: number): string[] {
    ...
    const rightTitle = selected
      ? theme.fg(
          "accent",
          theme.bold(
            `${statusWord(selected, theme)} · ${selected.title} · ${formatElapsed(selected)}`,
          ),
        )
    ...
    for (let i = 0; i < bodyHeight; i++) {
      lines.push(
        theme.fg("border", "│") +
          this.pad(leftContent[i] ?? "", leftWidth) +
          theme.fg("border", "│") +
          this.pad(rightContent[i] ?? "", rightWidth) +
          theme.fg("border", "│"),
      );
    }
    lines.push(
  ...
  private renderDetail(
    ...
    if (this.actionError)
      lines.push(
        theme.fg(
          "error",
          truncateToWidth(`Action: ${this.actionError}`, width),
        ),
      );
    if (approvals.length > 0) {
      ...
      lines.push(
        theme.fg(
          "warning",
          `Action: ${approvals.map((item) => item.operation).join(", ")}`,
        ),
      );
    }
    ...
    if (worktree?.branch)
      lines.push(theme.fg("muted", `Branch: ${worktree.branch}`));
    if (worktree?.path)
      lines.push(
        theme.fg(
          "muted",
          `Worktree: ${compactPath(worktree.path, width - 10)}`,
        ),
      );
    if (snap.backend === "orca") {
      if (snap.meta.nativeTerminalHandle)
        lines.push(
          theme.fg(
            "muted",
            `Orca terminal: ${compactPath(snap.meta.nativeTerminalHandle, width - 15)}`,
          ),
        );
      if (snap.meta.nativeTabId)
        lines.push(
          theme.fg(
            "muted",
            `Orca tab: ${compactPath(snap.meta.nativeTabId, width - 10)}`,
          ),
        );
      if (snap.meta.nativePaneKey)
        lines.push(
          theme.fg(
            "muted",
            `Orca pane: ${compactPath(snap.meta.nativePaneKey, width - 11)}`,
          ),
        );
      if (snap.meta.nativeWorktreeId)
        lines.push(
          theme.fg(
            "muted",
            `Orca worktree: ${compactPath(snap.meta.nativeWorktreeId, width - 15)}`,
          ),
        );
    }
    lines.push(...formatSubagentStatsLines(snap, width, theme));

    if (report) {
      lines.push(
        theme.fg("border", "─".repeat(Math.max(1, Math.min(width, 40)))),
      );
      lines.push(
        theme.fg(
          report.needsParentDecision ? "warning" : "text",
    ...
    if (snap.errorText) {
      lines.push(
        ...labeledLines(
          "Error: ",
          formatSubagentError(snap.errorText),
          width,
          3,
    ...
    if (inboxError)
      lines.push(
        ...labeledLines("Inbox: ", inboxError, width, 2).map((line) =>
          theme.fg("error", line),
        ),
      );
    ...
    lines.push(
      ...transcript.slice(
        Math.max(0, transcript.length - Math.max(1, height - lines.length)),
      ),
    );
  ...
  private renderRows(
    ...
    for (let i = 0; i < visible.length; i++) {
      ...
      const badges = [
        ...
        snap.errorText?.includes("recovery_required")
          ? theme.fg("error", "RECOVER")
          : "",
        snap.queued.length > 0
          ? theme.fg("warning", `Q${snap.queued.length}`)
          : "",
      ]
        .filter(Boolean)
        .join(" ");
...
class TakeoverView implements Component, Focusable {
  ...
  handleInput(data: string): void {
    ...
    const snap = this.snap();
    if (data === "r" && snap?.status === "failed" && this.options?.onRetry) {
      this.runAction(() => this.options!.onRetry!(snap.id), "Retry requested");
      return;
    }
    if (data === "a" && snap) {
    ...
    if (
      data === "i" &&
      snap?.backend === "orca" &&
      this.options?.onInspectTerminal
    ) {
      this.runAction(() => this.options!.onInspectTerminal!(snap));
      return;
    }
  ...
  render(width: number): string[] {
    ...
    const infoLines = buildSubagentInfoLines(
      snap,
      width,
      theme,
      this.options,
    ...
    const help = helpLines(
      `${configuredKeys(this.keybindings, "tui.input.submit")} send · ${configuredKeys(this.keybindings, "app.interrupt")} back · ${configuredKeys(this.keybindings, "app.clear")} abort · r retry · a approve/deliver · c confirm action · i inspect Orca · ${configuredKeys(this.keybindings, "tui.editor.pageUp")}/${configuredKeys(this.keybindings, "tui.editor.pageDown")} transcript`,
      width,
      compact ? 1 : 2,
    );
    const bodyHeight = Math.max(
      1,
      rows - 6 - infoLines.length - inputLines.length - help.length,
    );
    ...
    const header =
      theme.fg(
        "accent",
        theme.bold(
          `${statusWord(snap, theme)} · ${snap.title} · ${formatElapsed(snap)}`,
        ),
      ) +
      (this.options?.badge
    ...
    if (this.actionError) {
      body.push(
        truncateToWidth(
          theme.fg("error", `action error: ${sanitizeText(this.actionError)}`),
          width,
        ),
      );
    }
    ...
    if (this.scrollOffset > 0) {
      body.push(
        truncateToWidth(
          theme.fg(
            "dim",
            `... ${this.scrollOffset} newer lines below · page down`,
          ),
          width,
        ),
      );
