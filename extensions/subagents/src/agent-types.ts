...
import type { AgentConfig, BuiltinAgentType } from "./types.js";
...
let builtinPresets: ReadonlyMap<string, AgentPreset> | undefined;

/** Apply operational presets to the embedded default agents. */
export function setBuiltinPresets(presets: ReadonlyMap<string, AgentPreset> | undefined): void {
  builtinPresets = presets;
}

/**
 * Build a registry map: DEFAULT_AGENTS first,
...
export function buildAgentRegistry(userAgents: Map<string, AgentConfig>): Map<string, AgentConfig> {
  ...
  for (const [name, config] of DEFAULT_AGENTS) {
    const preset = builtinPresets?.get(name as BuiltinAgentType);
    registry.set(name, preset ? {
      ...config,
      model: preset.model ?? config.model,
      thinking: preset.thinking ?? config.thinking,
      maxTurns: preset.maxTurns ?? config.maxTurns,
    } : config);
  }
...
/**
 * Resolve a caller-supplied agent type against a registry. This is the single
 * decision point for every caller-supplied spawn — the Agent tool, workflows,
 * cross-extension RPC, and nested tools — so an invalid type never reaches
 * `runAgent`, where `getConfig` retains only a defensive fallback for internal
 * callers.
 *
 * Unknown, disabled, and case-ambiguous names are all rejected. Silent fallback
 * would turn a typo into a run with a different prompt and tool policy.
 *
