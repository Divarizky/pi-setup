...
import { isWorktreeIsolationEnabled } from "./core/worktree.js";
...
/** Hard ceiling on nesting: main session = 0, with four child levels allowed. */
export function getMaxSubagentDepth(): number { return 4; }
...
export function createNestedSubagentTools(context: NestedToolContext): ToolDefinition[] {
  const ACTIVE_CHILD_LIMIT_DEPTH = 2;
  const MAX_ACTIVE_CHILDREN = 2;
  const activeChildIds = new Set<string>();
  let pendingChildReservations = 0;

  const pruneCompletedChildren = (): void => {
    for (const id of activeChildIds) {
      ...
      if (record === undefined || (record.status !== "queued" && record.status !== "running")) {
        activeChildIds.delete(id);
      }
    }
  };

  const reserveChildSlot = (): boolean => {
    if (context.depth < ACTIVE_CHILD_LIMIT_DEPTH) return true;
    pruneCompletedChildren();
    if (activeChildIds.size + pendingChildReservations >= MAX_ACTIVE_CHILDREN) return false;
    pendingChildReservations += 1;
    return true;
  };

  const releaseChildReservation = (): void => {
    if (pendingChildReservations > 0) pendingChildReservations -= 1;
  };
  ...
  const agentTool = defineTool({
    ...
    description:
      ...
      "At grandchild depth and below, at most two children may be active at once.",
    ...
    execute: async (_toolCallId, params, signal, _onUpdate, ctx) => {
      ...
      // Strict resolution keeps a typo from selecting an agent outside this
      // parent's allowlist. The list stays allowlist-filtered so callers cannot
      // enumerate agents this parent may not reach.
      ...
      const childLimitApplies = context.depth >= ACTIVE_CHILD_LIMIT_DEPTH;
      if (childLimitApplies && !reserveChildSlot()) {
        return textResult(
          `Nested subagent launch ignored: this parent already has ${MAX_ACTIVE_CHILDREN} active children.`,
        );
      }
      let childReservationHeld = childLimitApplies;
      ...
      const attachTranscript = (id: string): void => {
        childId = id;
        if (childReservationHeld) {
          activeChildIds.add(id);
          releaseChildReservation();
          childReservationHeld = false;
        }
      ...
      try {
        ...
        // Keep the reservation and record even for a manager implementation
        // that does not invoke its optional onSpawned callback.
        if (childReservationHeld) {
          activeChildIds.add(record.id);
          releaseChildReservation();
          childReservationHeld = false;
        }
        return textResult(formatRecord(record, "inline"), record.status === "error");
      } catch (err) {
        if (childReservationHeld) releaseChildReservation();
