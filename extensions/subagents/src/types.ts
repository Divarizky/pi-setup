...
export type BuiltinAgentType = "explore" | "build" | "general";
export const BUILTIN_AGENT_TYPES = ["explore", "build", "general"] as const satisfies readonly BuiltinAgentType[];
...
export interface AgentConfig {
  ...
  /** Agent Control delivery policy for this agent type. */
  taskPolicy?: "scout" | "ship" | "flexible";
...
export interface AgentRecord {
  ...
  /** Agent Control task identity, when this is a durable top-level task. */
  taskId?: string;
  /** Agent Control task mode, when the feature is enabled for this spawn. */
  taskMode?: "scout" | "ship";
  /** Worktree info if the agent is running in an isolated worktree. */
  worktree?: { path: string; branch: string; baseSha: string; workPath: string };
  /** Worktree cleanup result after agent completion. */
  worktreeResult?: {
    status: "clean" | "candidate_ready" | "validation_failed" | "preservation_failed";
    hasChanges: boolean;
    branch?: string;
    path?: string;
    candidateSha?: string;
    changedFiles?: string[];
    validationEvidence?: import("./core/worktree.js").ValidationEvidence[];
    error?: string;
  };
