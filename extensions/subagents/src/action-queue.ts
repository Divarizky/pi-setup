...
function parseEvent(value: unknown): ActionEvent {
  ...
  return {
    ...
    ...(typeof value.evidence === "string" ? { evidence: value.evidence.slice(0, MAX_TEXT) } : {}),
  }
}
...
export class ActionQueue {
  ...
  async restore() {
    ...
    for (const line of raw.split(/\r?\n/).filter((item) => item.trim())) {
      ...
      try {
        ...
        throw new ActionQueueError("Action queue contains malformed JSON; refusing replay.")
      }
      if (isRecord(value) && value.kind === "action-event") {
        ...
        if (!current) throw new ActionQueueError(`Receipt has no matching action: ${receipt.actionId}`)
        if (current.receipt) continue
        this.records.set(receipt.actionId, { ...current, receipt, status: "confirmed" })
      }
    }
  }

  list(status?: ActionRecord["status"]) {
    return [...this.records.values()].filter((record) => !status || record.status === status)
  }
  ...
  async enqueue(event: Omit<ActionEvent, "kind">) {
    if (this.deletedJobs.has(event.jobId)) throw new ActionQueueError(`Job was deleted: ${event.jobId}`)
    ...
    const actionEvent: ActionEvent = {
      ...
      ...(event.evidence === undefined ? {} : { evidence: event.evidence.slice(0, MAX_TEXT) }),
    }
  ...
  async deleteJob(jobId: string): Promise<void> {
    ...
    for (const key of [...this.records.keys()]) {
      if (this.records.get(key)?.event.jobId === jobId) this.records.delete(key)
    }
    return this.enqueueWrite(async () => {
      const remaining = this.list().filter((record) => record.event.jobId !== jobId)
  ...
  async confirm(actionId: string): Promise<ActionRecord> {
    ...
    if (this.deletedJobs.has(current.event.jobId)) throw new ActionQueueError(`Job was deleted: ${current.event.jobId}`)
    if (current.status === "confirmed") return current
    return this.enqueueWrite(async () => {
      const latest = this.records.get(actionId)
      if (!latest) throw new ActionQueueError(`Unknown action: ${actionId}`)
      if (this.deletedJobs.has(latest.event.jobId)) throw new ActionQueueError(`Job was deleted: ${latest.event.jobId}`)
