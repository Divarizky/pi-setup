...
/**
 * Project/global default for writing a subagent's `.output` transcript.
 * A custom agent's `output_transcript` frontmatter still overrides it per run.
 */
export function getOutputTranscriptDefault(): boolean { return true; }
...
export function encodeCwd(cwd: string): string {
  return cwd
    ...
    .replace(/\./g, "-")           // dots → dash (kills ".." traversal)
    .replace(/[^A-Za-z0-9-]/g, "-") // anything else filesystem-hostile → dash
...
export function sessionTaskDir(cwd: string, sessionId: string): string {
  ...
  const root = join(tmpdir(), `subagents-${process.getuid?.() ?? 0}`);
