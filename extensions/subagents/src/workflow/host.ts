...
/** Cap on a gate command's length — a gate is a short check (`npm test`), not a script. */
export const MAX_GATE_COMMAND_LENGTH = 1024;

/** Cap on captured gate output — an unbounded transcript would bloat the journal. */
export const MAX_GATE_OUTPUT_LENGTH = 8 * 1024;
...
export function createWorkflowHost(deps: WorkflowHostOptions): WorkflowHost {
  ...
  async function executeGate(command: string, cwd: string): Promise<WorkflowGateResult> {
    // Trust-boundary guard: the command arrives from a model-written script.
    // Length-cap it, reject NUL bytes and multi-statement chaining up front,
    // and truncate captured output so a chatty gate can't bloat the journal.
    const trimmed = command.trim();
    if (!trimmed) return { ok: false, output: "Gate command is empty." };
    if (trimmed.length > MAX_GATE_COMMAND_LENGTH) {
      return { ok: false, output: `Gate command exceeds ${MAX_GATE_COMMAND_LENGTH} characters.` };
    }
    if (trimmed.includes("\0") || /[;|&`$]/.test(trimmed) || /\|\||&&|\$\(/.test(trimmed)) {
      return { ok: false, output: `Gate command uses shell chaining, which is not allowed: ${trimmed.slice(0, 120)}` };
    }
    const result = await pi.exec(GATE_SHELL[0], [GATE_SHELL[1], trimmed], {
      cwd,
    ...
    let output = [result.stdout, result.stderr]
      ...
      .filter(Boolean)
      .join("\n");
    if (output.length > MAX_GATE_OUTPUT_LENGTH) {
      output = output.slice(0, MAX_GATE_OUTPUT_LENGTH) + "\n...(gate output truncated)";
    }
