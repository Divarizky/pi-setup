...
export function resolveWorkflowSource(
  ...
  if (path !== undefined && path !== "") {
    ...
    // Same guards as the `name` path: reject symlinks, cap size before
    // reading into memory, and only accept real workflow scripts.
    try {
      if (statSync(resolved).size > MAX_SCRIPT_LENGTH) {
        return { ok: false, message: `Workflow script "${resolved}" exceeds ${MAX_SCRIPT_LENGTH} characters.` };
      }
    } catch (err) {
      return {
        ok: false,
    ...
    if (script === undefined) {
      return { ok: false, message: `Could not read workflow script "${resolved}": unreadable or a symlink.` };
    }
    if (!hasMetaDeclaration(script)) {
      return {
        ok: false,
        message: `"${resolved}" is not a workflow script — it has no \`export const meta = { name, description }\` declaration. Nothing was run.`,
      };
    }
