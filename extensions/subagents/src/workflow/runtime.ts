...
class Semaphore {
   ...
   *
   * Draining hands no permit over: every woken waiter re-checks
   * `aborted || settled` right after `acquire()` and releases immediately
   * (see the agentCall loop), so a drained wake-up can never run over-limit.
   */
  drain(): void {
...
export async function runWorkflow(options: RunWorkflowOptions): Promise<WorkflowRunResult> {
  ...
  return await new Promise<WorkflowRunResult>(resolve => {
    ...
    async function handleAgent(callId: number, payload: AgentCallPayload): Promise<void> {
      ...
      const agentType = resumed?.agentType ?? payload.agentType ?? "general";
