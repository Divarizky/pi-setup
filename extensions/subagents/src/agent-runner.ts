...
import { SCOUT_TOOL_NAMES, type TaskMode } from "./task-control/task-policy.js";
...
let defaultMaxTurns: number | undefined = 40;
 ...
 *
 * Exported because the widget's turn counter (`turn 3/20`) has to predict this
...
export interface RunOptions {
  ...
  signal?: AbortSignal;
  isolated?: boolean;
  /** Agent Control task mode; scout forces a read-only built-in tool set. */
  taskMode?: TaskMode;
...
export async function runAgent(
  ...
  // Resolve extensions/skills: isolated and Agent Control scout override extensions
  // to false so no extension tool can widen the read-only boundary.
  const scoutMode = options.taskMode === "scout";
  const extensions = options.isolated || scoutMode ? false : config.extensions;
  // Nulling excludes under isolated/scout also suppresses the orphaned-exclude
  // warning — both are intentional overrides, not a misconfiguration.
  const excludeExtensions = options.isolated || scoutMode ? undefined : config.excludeExtensions;
  ...
  let toolNames = scoutMode ? [...SCOUT_TOOL_NAMES] : getToolNamesForType(type);

  // Persistent memory: detect write capability and branch accordingly.
  // Account for disallowedTools — a tool in the base set but on the denylist is not truly available.
  if (agentConfig?.memory && !scoutMode) {
  ...
  const nestedRuntime = options.nestedRuntime && options.nestedRuntime.depth < effectiveMaxDepth
    ...
    : undefined;
  // Explore is a leaf agent and cannot delegate further.
  const nestedTools = agentConfig?.name !== "explore" && agentConfig?.allowedSubagents && nestedRuntime && !options.isolated
