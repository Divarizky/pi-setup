...
import { validateShipWorktree } from "../task-control/delivery-validation.js";
 ...
 */
// Foreground work has its own four-slot pool; background work uses maxConcurrent.
const DEFAULT_MAX_CONCURRENT_FOREGROUND = 4;
const MAX_EXPLORE_CONCURRENT = 4;
...
type Pool = "background" | "foreground" | "explore";
...
interface SpawnOptions {
   ...
   *
   * Used by internal UI dispatch and by the `/agents` agent-file generator,
   * which has no way to
  ...
  /** Agent Control durable task id. */
  taskId?: string;
  /** Agent Control task mode. Scout runs with a read-only tool boundary. */
  taskMode?: "scout" | "ship";
  /** Agent Control ship contract, consumed only inside the isolated worktree. */
  writeScope?: string[];
  validationCommands?: string[];
  /** Persist the worktree path before the agent receives control. */
  onWorktreeCreated?: (worktree: import("./worktree.js").WorktreeInfo) => void;
  /**
...
function formatWorktreePreservationFailure(result: WorktreeCleanupResult): string {
  const location = result.path ? ` Worktree retained at \`${result.path}\`.` : "";
  return `\n\n---\nWorktree preservation failed: ${result.error ?? "unknown error"}.${location}`;
}
...
export class AgentManager {
  ...
  private runningExplore = 0; // Explore has its own four-slot pool.
  ...
  private poolFor(record: AgentRecord): Pool | undefined {
    if (record.type === "explore") return "explore"; // Explore never consumes a background slot.
  ...
  private poolHasRoom(pool: Pool): boolean {
    if (pool === "explore") return this.runningExplore < MAX_EXPLORE_CONCURRENT;
  ...
  private async startAgent(
    ...
    const pool = this.poolFor(record);
    const releaseSlot = () => {
      if (pool === "explore") this.runningExplore--;
      else if (pool === "background") this.runningBackground--;
    ...
    if (options.taskMode === "ship" && !isWorktreeIsolationEnabled()) {
      releaseSlot();
      throw new Error("Agent Control ship task cannot start because worktree isolation is disabled.");
    }
    if (options.taskMode === "ship" && options.isolation !== "worktree") {
      releaseSlot();
      throw new Error('Agent Control ship task requires isolation: "worktree".');
    }
    ...
    if (options.isolation === "worktree" && isWorktreeIsolationEnabled()) {
      ...
      record.worktree = wt;
      // Durable ship tasks must publish the path before the child gets tools;
      // otherwise a restart during the run cannot classify the orphan safely.
      try {
        options.onWorktreeCreated?.(wt);
      } catch (error) {
        releaseSlot();
        throw error;
      }
    ...
    const promise = runAgent(ctx, type, prompt, {
      ...
      .then(async ({ responseText, session, aborted, steered, failure, structuredJson, structuredRetried }) => {
        ...
        // Opsi 2: max_turns abort preservasi result parsial + penanda — tanpa
        // LLM call tambahan (murah). Structured output dipisah, jadi tidak
        // dicemari tag prose.
        record.result = aborted && responseText
          ? `${responseText}\n\n---\n[partial — max turns exceeded]`
          : responseText;
        ...
        if (record.worktree) {
          ...
          const wtResult = options.taskMode === "ship"
            ? await cleanupWorktree(pi, baseCwd, record.worktree, options.description, {
                validate: worktree => aborted || record.status === "stopped"
                  ? Promise.resolve({
                      passed: false,
                      changedFiles: [],
                      evidence: [],
                      error: "Ship task was aborted before validation completed.",
                    })
                  : validateShipWorktree(pi, worktree, options.writeScope ?? ["**"], options.validationCommands ?? []),
              })
            : await cleanupWorktree(pi, baseCwd, record.worktree, options.description);
          record.worktreeResult = wtResult;
          if (wtResult.status !== "preservation_failed" && wtResult.hasChanges && wtResult.branch) {
          ...
          } else if (wtResult.status === "validation_failed") {
            const location = wtResult.path ? ` Worktree preserved at \`${wtResult.path}\`.` : "";
            record.result = (record.result ?? "") +
              `\n\n---\nShip validation failed: ${wtResult.error ?? "validation did not pass."}${location}`;
          } else if (wtResult.status === "preservation_failed") {
            record.result = (record.result ?? "") + formatWorktreePreservationFailure(wtResult);
          }
        }
      ...
      .catch(async (err) => {
        ...
        if (record.worktree) {
          try {
            const wtResult = options.taskMode === "ship"
              ? await cleanupWorktree(pi, baseCwd, record.worktree, options.description, {
                  validate: _worktree => Promise.resolve({
                      passed: false,
                      changedFiles: [],
                      evidence: [],
                      error: record.error ?? "Agent failed before ship validation.",
                    }),
                })
              : await cleanupWorktree(pi, baseCwd, record.worktree, options.description);
            record.worktreeResult = wtResult;
            if (wtResult.status === "preservation_failed") {
              record.error = `${record.error}\n${formatWorktreePreservationFailure(wtResult).trim()}`;
            } else if (wtResult.status === "validation_failed") {
              record.error = `${record.error}\nShip validation failed: ${wtResult.error ?? "validation did not pass."}${wtResult.path ? ` Worktree preserved at ${wtResult.path}.` : ""}`;
            }
  ...
  async resume(
    ...
    // Never re-enter a run still in flight (same guard as the background
    // path above): a second run would overwrite record.abortController,
    // orphaning the live run beyond stop/abortAll reach.
    if (record.status === "running" || record.status === "queued") return undefined;
    record.status = "running";
    record.startedAt = Date.now();
    record.completedAt = undefined;
  ...
  private startResume(
    ...
    record.status = "running";
    record.startedAt = Date.now();
    // Explore is counted by poolFor, so it never leaks a background slot.
    {
      const p = this.poolFor(record);
      if (p === "explore") this.runningExplore++;
      else if (p === "background") this.runningBackground++;
      else if (p === "foreground") this.runningForeground++;
    }
    this.onStart?.(record);
