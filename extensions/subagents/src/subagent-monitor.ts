...
/** Backward-compatible alias for the canonical backend status type. */
export type SemanticStatus = SessionSemanticStatus;
...
export function classifyStatus(
  ...
  if (snapshot.status === "failed") return "dead";
  ...
  return {
    ...
    meta: {
      ...
      ...(snapshot.meta.worktree
        ? { worktree: { ...snapshot.meta.worktree } }
        : {}),
    },
  };
}
...
function evidenceActionId(
  evidence: ExternalStatusEvidence,
  type: ActionEventType,
) {
...
export class SubagentMonitor {
  ...
  constructor(queue: ActionQueue, options: SubagentMonitorOptions = {}) {
    this.queue = queue;
  ...
  async observe(
    snapshot: SubagentSnapshot,
  ): Promise<ReadonlyArray<ActionRecord>> {
      ...
      if (snapshot.errorText?.includes("restarted")) {
        return [
          await this.enqueue(
            snapshot,
            "recovery_required",
            "Job requires recovery after restart.",
          ),
        ];
      }
      return [];
    ...
    if (previousSnapshot.status !== "done" && snapshot.status === "done") {
      actions.push(
        await this.enqueue(
          snapshot,
          "job_settled",
          "Subagent completed and requires review.",
        ),
      );
      if (snapshot.meta.mode === "build" && snapshot.meta.worktree) {
        actions.push(
          await this.enqueue(
            snapshot,
            "approval_required",
            "Build delivery requires explicit approval.",
          ),
        );
      }
    } else if (
      previousSnapshot.status !== "failed" &&
      snapshot.status === "failed"
    ) {
      const recoveryRequired =
        snapshot.errorText?.toLowerCase().includes("recovery_required") ===
        true;
      actions.push(
        await this.enqueue(
          snapshot,
      ...
      actions.push(
        await this.enqueue(
          snapshot,
          "status_unknown",
          "Subagent status is stale or unverifiable.",
        ),
      );
    }
    return actions;
  ...
  async observeEvidence(
    ...
    const localStatus = snapshot
      ? classifyStatus(snapshot, now, this.staleAfterMs)
      : undefined;
    const conflicting =
      !stale &&
      !identityMismatch &&
    ...
    const effectiveStatus: SemanticStatus =
      stale || identityMismatch || conflicting ? "unknown" : evidence.status;
  ...
  async reconcile(snapshots: ReadonlyArray<SubagentSnapshot>) {
    ...
    for (const snapshot of snapshots)
      actions.push(...(await this.observe(snapshot)));
    return actions;
  }
