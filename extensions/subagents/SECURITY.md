...
This document explains the security model behind `subagents` and
where the boundaries are.

`subagents` is a [pi](https://pi.dev) extension. It spawns and orchestrates
...
unless they demonstrate how `subagents` grants that write access or crosses an
operating-system privilege boundary.

`subagents` relies on the user only loading trustworthy agent definitions
...
If you believe you found a security vulnerability in `subagents`, report it
privately to the maintainer through the personal pi-setup repository. Do not
publish sensitive details in a public issue or commit.
...
in scope — for example, a flaw in `subagents` that crosses an
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  profiles, or dotfile managers, unless the report shows how `subagents`
...
affected and that the issue is reachable through `subagents`.
