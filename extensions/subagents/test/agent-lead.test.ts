...
test("Lead Agent registry survives restart and updates its current job", async () => {
  ...
  try {
    ...
    await store.update("lead-docs", {
      jobId: "lead-docs-job-2",
      lastSummary: "token=super-secret Previous investigation complete.",
    });
    const restored = new LeadAgentStore(root);
    await restored.restore();
    assert.equal(restored.get("lead-docs")?.jobId, "lead-docs-job-2");
    assert.equal(
      restored.get("lead-docs")?.charter,
      "Own documentation investigations.",
    );
    assert.equal(
      restored.get("lead-docs")?.scope,
      "Documentation and API reference work.",
    );
    assert.equal(
      restored.get("lead-docs")?.lastSummary,
      "token=[redacted] Previous investigation complete.",
    );
  } finally {
...
test("Lead Agent registry migrates legacy resident state", async () => {
  ...
  try {
    await writeFile(
      path.join(root, "resident-subagents.json"),
      JSON.stringify({
        version: 1,
        residents: [
          {
