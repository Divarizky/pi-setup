...
test("job queue orders ready jobs and rejects dependency cycles", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      queue.ready((id) => id === "first").map((job) => job.id),
      ["third", "second"],
    );
  } finally {
...
test("removing a dependency blocks dependents instead of leaving an orphan", async () => {
  const root = await mkdtemp(
    path.join(os.tmpdir(), "pi-job-delete-dependency-"),
  );
  try {
    const queue = new JobQueue(root);
    await queue.enqueue({
    ...
    assert.match(
      queue.get("second")?.errorText ?? "",
      /Dependency first was deleted/,
    );
  } finally {
    await rm(root, { recursive: true, force: true });
  }
...
test("job queue restores durable records fail closed for active jobs", async () => {
  ...
  try {
    ...
    assert.equal(restored.get("persisted")?.task.prompt, "inspect");
    assert.equal(restored.get("persisted")?.task.cwd, process.cwd());
    assert.match(restored.get("waiting")?.errorText ?? "", /re-enqueue/);
    const requeued = await restored.requeue("persisted", {
      ...restored.get("persisted")!.task,
      prompt: "resume explicitly",
    });
    assert.equal(requeued.status, "queued");
    assert.equal(requeued.task.prompt, "resume explicitly");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
