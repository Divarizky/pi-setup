...
test("subagent monitor emits durable action events for settlement and failure", async () => {
  const root = await mkdtemp(
    path.join(os.tmpdir(), "pi-subagent-monitor-settlement-"),
  );
  try {
    ...
    assert.deepEqual(
      actions.map((item) => item.event.type),
      ["job_settled", "approval_required"],
    );
...
test("monitor preserves prior status when manager mutates a snapshot in place", async () => {
  const root = await mkdtemp(
    path.join(os.tmpdir(), "pi-subagent-monitor-mutable-"),
  );
  try {
    const monitor = new SubagentMonitor(new ActionQueue(root), {
      now: () => 1_000,
    });
...
test("external dead evidence creates one durable session action", async () => {
  const root = await mkdtemp(
    path.join(os.tmpdir(), "pi-subagent-monitor-external-"),
  );
  try {
    const monitor = new SubagentMonitor(new ActionQueue(root), {
      now: () => 1_000,
    ...
    assert.equal(
      (await monitor.observeEvidence(evidence))[0]?.event.type,
      "session_dead",
    );
...
test("subagent monitor detects stale evidence and restart errors", async () => {
  const root = await mkdtemp(
    path.join(os.tmpdir(), "pi-subagent-monitor-status-"),
  );
  try {
    const queue = new ActionQueue(root);
    const monitor = new SubagentMonitor(queue, {
      now: () => 1_000,
      staleAfterMs: 100,
    });
    assert.equal(
    ...
    const unknown = await monitor.observe(
      snapshot({
        metrics: {
          runCount: 1,
          restartCount: 0,
    ...
    const recovered = await new SubagentMonitor(
      new ActionQueue(path.join(root, "recovered")),
      { now: () => 1_000 },
    ).observe(
      snapshot({
        status: "failed",
    ...
    const transitionedMonitor = new SubagentMonitor(
      new ActionQueue(path.join(root, "transitioned")),
      { now: () => 1_000 },
    );
    await transitionedMonitor.observe(snapshot());
    const transitioned = await transitionedMonitor.observe(
      snapshot({
        status: "failed",
    ...
    const conflictMonitor = new SubagentMonitor(
      new ActionQueue(path.join(root, "conflict")),
      { now: () => 1_000 },
    );
    ...
    const mismatchMonitor = new SubagentMonitor(
      new ActionQueue(path.join(root, "mismatch")),
      { now: () => 1_000 },
    );
