...
function harness(
  ...
  let footerFactory: ((tui: any, theme: any, footerData: any) => { render(w: number): string[] }) | undefined;
  let editorText = "";
  let opened = false;
  let closed = false;
  ...
  const ui = {
    setFooter: (factory: unknown) => { footerFactory = factory as any; },
    // Test-only: FleetList captures tui via footer factory closure; expose it
    // so focus tests can inject a focusedComponent before pressing keys.
    captureTui: (tui: unknown) => { (fleet as unknown as { tui: unknown }).tui = tui; },
  ...
  return {
    ...
    render: (width = 120) => (footerFactory ? footerFactory(fakeTui, theme, footerData).render(width) : []),
...
describe("FleetList navigation", () => {
  ...
  it("re-arms the refresh timer when the list is re-shown (toggle off→on)", () => {
    ...
    try {
      ...
      fleet.setUICtx({
        setFooter: () => {}, onTerminalInput: () => () => {}, getEditorText: () => "",
...
describe("FleetList vs other focused components (#123)", () => {
  ...
  /** Hand the fleet list its `tui` (happens on first footer render in pi) with the given focus. */
  function focusInHarness(h: Harness, focused: unknown): void {
    (h.ui as unknown as { captureTui(tui: unknown): void }).captureTui(h.widgetTui);
...
describe("FleetList cost display", () => {
  ...
  function row(showCost: boolean, cost: number, activity?: Map<string, AgentActivity>): string {
    ...
    fleet.update();
    const footerData = { getGitBranch: () => null, getExtensionStatuses: () => new Map(), getAvailableProviderCount: () => 0 };
    return factory({ requestRender: () => {}, terminal: { columns: 120, rows: 40 } }, theme, footerData).render(120).join("\n");
  }
