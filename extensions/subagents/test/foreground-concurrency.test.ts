...
const fg = (manager: AgentManager, prompt: string, options: any = {}) =>
  manager.spawnAndWait(mockPi, mockCtx, "general", prompt, {
    description: prompt,
    ...options,
  });

const bg = (manager: AgentManager, prompt: string, options: any = {}) =>
  manager.spawn(mockPi, mockCtx, "general", prompt, {
    description: prompt,
    isBackground: true,
    ...options,
  });
...
describe("maxConcurrentForeground", () => {
  ...
  it.skip("is unlimited by default, and the default path stays synchronous (ponytail: FG hardcoded 4", () => {
    controllableRuns();
  ...
  it("does not queue a detached spawn, even one flagged isBackground: false", () => {
    ...
    manager.spawn(mockPi, mockCtx, "general", "rpc", {
      description: "rpc",
      isBackground: false,
    });
  ...
  describe("cancellation", () => {
    ...
    it.skip("stops an immediate spawn whose signal is already aborted, pool off (ponytail: FG hardcoded 4", async () => {
      controllableRuns();
      manager = new AgentManager();
      expect(manager.getMaxConcurrentForeground()).toBe(0);
  ...
  it("fires each caller's onSpawned hook when its own deferred spawn starts", async () => {
    ...
    const first = manager.spawnAndWait(
      mockPi, mockCtx, "general", "a", { description: "a" }, firstHook,
    );
    const second = manager.spawnAndWait(
      mockPi, mockCtx, "general", "b", { description: "b" }, secondHook,
    );
