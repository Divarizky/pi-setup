...
function callForeground(tools: Map<string, any>, prompt: string, opts: {
  ...
  return tools.get("Agent").execute(
    `tc-${prompt}`,
    { prompt, description: prompt, subagent_type: "general", run_in_background: false },
    opts.signal,
    opts.onUpdate,
    ctx(),
  );
...
describe("maxConcurrentForeground, through the Agent tool", () => {
  it.skip("runs blocking calls one at a time and returns each its own result (ponytail: FG hardcoded 4, re-tune pending", async () => {
    const { tools } = boot();
  ...
  it.skip("resolves a queued call as STOPPED when the turn is interrupted (ponytail: FG hardcoded 4", async () => {
    const { tools } = boot();
    const resolvers = controllableRuns();
  ...
  it.skip("says it is queued in the live tool result, then stops saying it (ponytail: FG hardcoded 4", async () => {
    const { tools } = boot();
    const resolvers = controllableRuns();
