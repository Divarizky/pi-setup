...
describe("Agent tool result — effective model", () => {
  ...
  it("reports the session's level, and what was asked for, when pi clamps it", async () => {
    ...
    const result = await tool.execute(
      "tc-3",
      { prompt: "go", description: "d", subagent_type: "general", thinking: "max", run_in_background: false },
      undefined,
      vi.fn(),
      ctx(),
    );
  ...
  it("says nothing about a request that was honored", async () => {
    ...
    const result = await tool.execute(
      "tc-6",
      { prompt: "go", description: "d", subagent_type: "general", thinking: "high", run_in_background: false },
      undefined,
      vi.fn(),
      ctx(),
    );
...
describe("Agent tool result — resume", () => {
  it("renders the reopened session's settings, not the resume call's", async () => {
    ...
    const first = await tool.execute(
      "tc-7",
      { prompt: "go", description: "original", subagent_type: "general", run_in_background: false },
      undefined,
      vi.fn(),
      context,
    );
