...
describe("detectEnv", () => {
  it("detects git repo in current project", async () => {
    // Hermetic: the working copy here is not a git checkout, so detect against
    // a real temp repo instead of asserting on process.cwd().
    const dir = mkdtempSync(join(tmpdir(), "env-git-"));
    try {
      execSync("git init", { cwd: dir, stdio: "pipe" });
      const env = await detectEnv(mockPi(), dir);
