...
describe("handleBase", () => {
  ...
  it("keeps a hyphenated type as-is", () => {
    expect(handleBase("general")).toBe("general");
  });
  ...
  it("only ever produces handles the suggestion trigger can match", () => {
    for (const type of ["explore", "general", "Code Review!", "!!!", "デバッグ"]) {
...
describe("resolveHandleToType", () => {
  const TYPES = ["general", "explore", "Code Review!"];
...
describe("agentMentionReminder", () => {
  ...
  it("names the agent it was given", () => {
    expect(agentMentionReminder("general")).toContain('invoke the agent "general"');
  });
});
