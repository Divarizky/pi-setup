...
describe("script globals", () => {
  it("runs a script, returns its value and reports meta", async () => {
    ...
    expect(calls[0].agentType).toBe("general");
...
describe("Claude Code option compatibility", () => {
  ...
  it("accepts every option a Claude Code script actually uses", async () => {
    const stub = stubHost();
    const result = await run(
      "return await agent('go', { label: 'L', phase: 'P', agentType: 'general', model: 'haiku', effort: 'high', isolation: 'worktree' });",
      { host: stub.host },
    );

    expect(result.status).toBe("completed");
    expect(stub.calls[0]).toMatchObject({ label: "L", agentType: "general", effort: "high" });
  });
