...
describe("Agent tool — background spawn", () => {
   ...
   * published on `Symbol.for("subagents:manager")` exposes no way to clear
   ...
   */
  bench(
    "general, run_in_background",
    async () => {
      await agent.execute(
        `bench-${i++}`,
        { subagent_type: "general", description: "bench spawn", prompt: "go", run_in_background: true },
        undefined,
        undefined,
        c,
      );
