...
test("deleting a job removes its durable history but preserves other jobs", async () => {
  ...
  try {
    ...
    await store.upsert({ jobId: "delete-me", title: "delete", mode: "build", cwd: "/a", status: "done", createdAt: 1 })
    await store.upsert({ jobId: "keep-me", title: "keep", mode: "build", cwd: "/b", status: "done", createdAt: 2 })
    ...
    assert.deepEqual((await store.load()).map((job) => job.jobId), ["keep-me"])
    assert.deepEqual(await store.loadEvents(), [{ at: 4, jobId: "keep-me", event: "done" }])
  } finally {
    await rm(root, { recursive: true, force: true })
  }
})
...
test("deleted jobs cannot be recreated by late persistence callbacks", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "pi-job-state-delete-race-"))
  try {
    const store = new JobPersistence(root)
    await store.deleteJob("job-1")
    await store.upsert({ jobId: "job-1", title: "late", mode: "build", cwd: "/a", status: "done", createdAt: 1 })
...
test("approval state survives a new store instance", async () => {
  ...
  try {
    ...
    assert.deepEqual(await new JobPersistence(root).loadApprovals(), [approval])
  } finally {
    await rm(root, { recursive: true, force: true })
  }
})
...
test("concurrent job and approval writes preserve every latest record", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "pi-job-state-concurrent-"))
  try {
    const store = new JobPersistence(root)
    await Promise.all([
      store.upsert({ jobId: "job-a", title: "a", mode: "build", cwd: "/a", status: "done", createdAt: 1 }),
      store.upsert({ jobId: "job-b", title: "b", mode: "scout", cwd: "/b", status: "done", createdAt: 2 }),
      store.saveApprovals([{
        id: "approval:a:commit", jobId: "a", operation: "commit", status: "pending", requestedAt: 1,
      }]),
      store.saveApprovals([{
        id: "approval:b:commit", jobId: "b", operation: "commit", status: "pending", requestedAt: 2,
      }]),
    ])
