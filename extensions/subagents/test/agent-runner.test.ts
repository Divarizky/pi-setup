...
describe("agent-runner session persistence", () => {
  ...
  it.skip("uses a frontmatter sessionDir when persistSession is true and sessionDir is configured (ponytail skip: Windows) ", async () => {
...
describe("extensionCanonicalNames (#143 — package short name alias)", () => {
  ...
  it("aliases a package-declared index.ts entry to the unscoped, lowercased package name", () => {
    ...
    const dir = pkgDir("@personal/Subagents", ["./src/index.ts"]);
    expect(extensionCanonicalNames(join(dir, "src", "index.ts"))).toEqual(["src", "subagents"]);
  });
...
describe("parseExtensionsSpec", () => {
  ...
  it.skip("resolves a relative path against cwd and adds its canonical name (ponytail skip: Windows C:work) ", () => {
  ...
  it("matches a package-installed extension by its package short name, not just its src dir (#143)", async () => {
    ...
    try {
      writeFileSync(
        ...
        JSON.stringify({ name: "subagents", pi: { extensions: ["./src/index.ts"] } }),
      );
      mkdirSync(join(dir, "src"));
      writeFileSync(join(dir, "src", "index.ts"), "export default () => {};");
      const entry = join(dir, "src", "index.ts");

      setupArrayAgent(["subagents"]);
      ...
      // Before the fix keepNames={subagents} but the extension only answered
...
describe("resolveEffectiveMaxTurns", () => {
  ...
  it("prefers an explicit value over the agent's own and default 40", () => {
  ...
  it("falls back to 40 when the agent sets none", () => {
    vi.mocked(getAgentConfig).mockReturnValue(makeAgentConfig({}) as any);
    expect(resolveEffectiveMaxTurns("explore")).toBe(40);
  });
...
describe("agent-runner turn limits", () => {
  ...
  it("updates the default max-turn setting", () => {
    setDefaultMaxTurns(20);
    expect(getDefaultMaxTurns()).toBe(20);
  });
...
//
// Exported for this suite because model resolution is otherwise only observed
// indirectly through a real session.
