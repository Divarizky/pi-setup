...
test("workflow events fold through blocked recovery to done", () => {
  ...
  const events: WorkflowEvent[] = [
    ...
    { type: "status", status: "blocked", message: "Needs clarification", at: 120 },
...
test("workflow rejects invalid transitions instead of reviving a settled task", () => {
  ...
  const working = applyWorkflowEvent(task, { type: "status", status: "working", at: 105 })
  const done = applyWorkflowEvent(working, { type: "status", status: "done", at: 110 })
