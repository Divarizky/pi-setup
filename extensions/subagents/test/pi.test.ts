 ...
 * - cancel sets status to error/interrupted
 */
...
const createRuntime = () =>
  ManagedRuntime.make(SubagentManagerLive.pipe(Layer.provide(PiOnlyRegistryLive)))
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(done.status, "done", `status should be done, got: ${done.status} (${done.errorText})`)
      assert.ok(
...
test(
  "pi: cancel settles a running session as error",
  { timeout: TIMEOUT_MS },
  async () => {
    const runtime = createRuntime()
    try {
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const done = manager.view.get(snap.id)
      assert.ok(done, "snapshot must exist after waitFor")
      assert.ok(done.turns != null && done.turns >= 1, `at least 1 turn; got ${done.turns}`)
      assert.ok(
