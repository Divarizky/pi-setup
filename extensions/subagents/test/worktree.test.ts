...
describe("worktree", () => {
  ...
  describe("createWorktree", () => {
    ...
    it.skip("workPath equals path when created from the repo root (ponytail skip: Windows", async () => {
      const wt = (await createWorktree(pi, repoDir, "root-wp"))!;
      expect(wt.workPath).toBe(wt.path);
      try { execFileSync("git", ["worktree", "remove", "--force", wt.path], { cwd: repoDir, stdio: "pipe" }); } catch { /* ignore */ }
    });

    it.skip("workPath preserves subdirectory scoping (monorepo package cwd) (ponytail skip: Windows", async () => {
...
describe("cleanupWorktree — failure path", () => {
  ...
  it.skip("swallows a git failure inside a still-present worktree and reports no changes (ponytail skip: Windows", async () => {
  ...
  it("preserves the worktree and reports preservation_failed when the commit fails", async () => {
    // `git commit` failing resolves with a non-zero code rather than throwing.
    // The staged worktree must remain available for recovery.
  ...
  it("preserves the worktree and reports preservation_failed when branch creation fails", async () => {
  ...
  it("keeps the worktree when candidate SHA verification fails", async () => {
    const wt = (await createWorktree(pi, repoDir, "sha-fails"))!;
    writeFileSync(join(wt.path, "work.txt"), "agent output");

    const result = await cleanupWorktree(
      failingPi(
        args => args[0] === "rev-parse" && args[1] === "refs/heads/pi-agent-sha-fails",
        { code: 1, killed: false },
      ),
      repoDir,
      wt,
