...
test("createBranchName follows conventional readable naming", () => {
  assert.equal(
    createBranchName("Recover missing session", {
      type: "fix",
      scope: "subagents",
    }),
    "fix/subagents/recover-missing-session",
  );
  assert.equal(createBranchName("Add docs"), "chore/add-docs");
  assert.equal(createBranchName("Add docs", { scope: "" }), "chore/add-docs");
});
...
test("createJobId produces a safe, unique subagent job id", () => {
  ...
  assert.match(
    createLeadJobId("Documentation Lead"),
    /^lead-documentation-[a-z0-9-]+$/,
  );
  assert.match(
    createLeadJobId("Documentation Lead", "follow-up"),
    /^lead-documentation-follow-up-[a-z0-9-]+$/,
  );
...
test("retirement removes only a verified clean worktree", async () => {
  ...
  try {
    ...
    assert.equal(worktree.branch, "chore/retire-test");

    await removeSubagentWorktree(worktree);
    await assert.rejects(
      readFile(path.join(worktree.path, "README.md"), "utf8"),
    );
    assert.equal(
      (await git(repoRoot, ["status", "--porcelain"])).stdout.trim(),
      "",
    );
  } finally {
...
test("explicit cascade deletion can remove a dirty worktree", async () => {
  ...
  try {
    ...
    await assert.rejects(
      readFile(path.join(worktree.path, "uncommitted.txt"), "utf8"),
    );
    assert.equal(
      (
        await git(repoRoot, ["branch", "--list", worktree.branch])
      ).stdout.trim(),
      "",
    );
...
test("createSubagentWorktree isolates a subagent checkout from the source repo", async () => {
  const tempRoot = await mkdtemp(
    path.join(os.tmpdir(), "pi-subagent-worktree-"),
  );
  const repoRoot = path.join(tempRoot, "repo");
  const workspaceRoot = path.join(tempRoot, "workspace");
  ...
  try {
    ...
    assert.equal(
      (
        await readFile(path.join(worktree.path, "README.md"), "utf8")
      ).replaceAll("\r\n", "\n"),
      "source\n",
    );
    assert.equal(
      (await git(worktree.path, ["branch", "--show-current"])).stdout.trim(),
      worktree.branch,
    );
    assert.equal(
      await realpath(
        (
          await git(worktree.path, ["rev-parse", "--show-toplevel"])
        ).stdout.trim(),
      ),
    ...
    await writeFile(
      path.join(worktree.path, "subagent.txt"),
      "subagent output\n",
      "utf8",
    );
