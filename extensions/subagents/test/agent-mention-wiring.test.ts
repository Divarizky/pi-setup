...
afterEach(async () => {
  ...
  delete (globalThis as any)[Symbol.for("subagents:manager")];
  booted = undefined;
  hermetic?.restore();
  hermetic = undefined;
});
...
describe("messaging a running agent", () => {
  ...
  it("un-consumes the result so the agent's reply is still relayed", async () => {
    ...
    await flush();
    const record = (globalThis as any)[Symbol.for("subagents:manager")].getRecord(id);
...
describe("messaging a finished agent", () => {
  ...
  it("honours the agent's output_transcript: false when resuming", async () => {
    ...
    const record = b.pi.__manager?.getRecord?.(id)
      ?? (globalThis as any)[Symbol.for("subagents:manager")].getRecord(id);
...
describe("resolving which agent a handle means", () => {
  const managerRegistry = () => (globalThis as any)[Symbol.for("subagents:manager")];
  ...
  it("queues the message for an agent still waiting on a concurrency slot", async () => {
    ...
    await spawnBackground(tools, "general");
    const queuedId = await spawnBackground(tools, "general");
    await flush();

    const queued = managerRegistry().getRecord(queuedId);
    expect(queued.status).toBe("queued");

    expect(await send(lifecycle, "@general-2 wait for me")).toEqual({ action: "handled" });
...
describe("mentioning an agent that has never run", () => {
  ...
  it("runs it in the background so the prompt is not blocked", async () => {
    ...
    await send(lifecycle, "@explore go");
    const record = (globalThis as any)[Symbol.for("subagents:manager")]
...
describe("letting a clone of the conversation start the agent", () => {
  ...
  it("hands the clone the resolved type, the message and the real Agent tool", async () => {
    ...
    await send(lifecycle, "@general sketch the migration");
    await flush();

    expect(runMentionClone).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "general", // ponytail: Plan removed
...
describe("@agent-<type> — Claude Code's manual spelling", () => {
  ...
  it("prefers an agent literally named agent-<x> over the unwrapped spelling", async () => {
    ...
    await tools.get("Agent").execute(
      "tc-named",
      { prompt: "go", description: "named", subagent_type: "general", name: "agent-explore", run_in_background: true },
...
describe("resuming an evicted agent by name", () => {
  ...
  async function evict(id: string) {
    const manager = (globalThis as any)[Symbol.for("subagents:manager")];
  ...
  it("refuses to reopen a conversation under a substitute agent", async () => {
    // resolveSpawnType rejects a type it cannot resolve — and "cannot
    // resolve" includes merely disabled, which `/agents → Disable` does at
    // any time. Reopening this under a substitute agent would instead use
    // an explore transcript under general's prompt and tools while
