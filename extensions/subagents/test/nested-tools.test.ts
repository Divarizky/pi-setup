...
describe("child-safe nested Agent tools", () => {
  ...
  it("ignores launches beyond two active children from grandchild depth", async () => {
    const [agent] = tools("all", 2, 4);
    const params = {
      subagent_type: "scout",
      description: "find files",
      prompt: "Find them",
    ...
    await execute(agent, params);
    await execute(agent, { ...params, prompt: "Find more" });
    const ignored = await execute(agent, { ...params, prompt: "Find even more" });

    expect(ignored.isError).toBe(false);
    expect(ignored.content[0].text).toContain("ignored");
    expect(spawn).toHaveBeenCalledTimes(2);

    records.get("child-1").status = "completed";
    await execute(agent, { ...params, prompt: "Find again" });
    expect(spawn).toHaveBeenCalledTimes(3);
  });

  it("does not apply the active-child cap before grandchild depth", async () => {
    const [agent] = tools("all", 1, 4);
    const params = {
      subagent_type: "scout",
      description: "find files",
      prompt: "Find them",
    ...
    await execute(agent, params);
    await execute(agent, { ...params, prompt: "Find more" });
    await execute(agent, { ...params, prompt: "Find even more" });
  ...
  it("rejects unknown nested types before spawning", async () => {
    const [agent] = tools(["scout"]);
    const result = await execute(agent, {
  ...
  it("files a nested transcript under the root session, honoring output_transcript", async () => {
    ...
    const transcriptRoot = join(tmpdir(), `subagents-${process.getuid?.() ?? 0}`, encodeCwd(cwd));
    try {
      const [agent] = tools();
