...
describe("encodeCwd", () => {
  ...
  it("neutralizes dot segments so traversal cannot survive encoding", () => {
    expect(encodeCwd("/home/user/../etc")).toBe("home-user-etc");
    expect(encodeCwd("/tmp/.../x")).not.toContain("..");
  });

  it("strips characters hostile to filesystems", () => {
    expect(encodeCwd("/home/user name@host:proj")).toBe("home-user-name-host-proj");
  });
});
