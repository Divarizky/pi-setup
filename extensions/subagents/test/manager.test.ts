...
test("restored jobs remain visible and running jobs fail closed as orphaned", async () => {
  await withManager(async (manager, runtime) => {
    ...
    await runTool(runtime, manager.restore(jobs, [
    ...
    assert.equal(manager.view.get("sa-7")?.meta.nativeTerminalHandle, "term-restored")
    assert.equal(manager.view.get("sa-7")?.meta.nativeWorktreeId, "wt-restored")
    assert.deepEqual(manager.view.get("sa-7")?.eventLog.slice(0, 2), [
      { at: 15, event: "spawned" },
      { at: 19, event: "settled", message: "done" },
    ])
    assert.equal(manager.view.get("sa-8")?.status, "error")
    ...
    const next = await runTool(runtime, manager.spawn("pi", task("avoid restored id")))
...
test("failed subagents retry with bounded recovery", async () => {
  await withManager(async (manager, runtime) => {
    const snap = await runTool(runtime, manager.spawn("pi", task("FAIL: first attempt")))
...
test("stub subagent completes and delivers a final result", async () => {
  await withManager(async (manager, runtime) => {
    ...
    assert.match(done.finalText, /\[stub:pi\] completed: Say hello to the tests/)
...
test("recovery-required preserves the job and permits a bounded retry", async () => {
  await withManager(async (manager, runtime) => {
    const snap = await runTool(runtime, manager.spawn("pi", task("Recover after terminal loss")))
    await runTool(runtime, manager.markRecoveryRequired(snap.id, "terminal disconnected"))
    const dead = manager.view.get(snap.id)
    assert.equal(dead?.status, "error")
    assert.match(dead?.errorText ?? "", /recovery_required/)
    assert.ok(dead?.eventLog.some((event) => event.event === "RecoveryRequired"))

    await runTool(runtime, manager.retry(snap.id, "Continue from the preserved worktree."))
    await runTool(runtime, manager.waitFor([snap.id]))
...
test("spawn origin propagates to ids, snapshots, and settlement", async () => {
  await withManager(async (manager, runtime) => {
    ...
    const model = await runTool(runtime, manager.spawn("pi", task("model task")))
...
test("the global concurrency cap includes quick-ask sessions", async () => {
  await withManager(async (manager, runtime) => {
    ...
    await assert.rejects(
      runTool(
        runtime,
        manager.spawn("pi", { ...task("another side question"), origin: "quick-ask" }),
      ),
      /Max 4 subagents/,
    )
  })
