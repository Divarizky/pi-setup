 ...
 * agent-startup-error) mock ../src/core/worktree.js entirely and assert only the gate
...
// Real pi sessions and inline dependency transforms are slow on Windows.
// Keep this timeout local to the heavy end-to-end suite.
  ...
  afterEach(async () => {
    ...
    for (const dir of repos.splice(0)) {
      ...
      try {
        ...
        try {
          const { execSync: _exec } = require("node:child_process");
          _exec('rmdir /s /q "' + dir.replace(/"/g,'""') + '"', { stdio: "ignore" });
        } catch {}
      }
    }
  });
