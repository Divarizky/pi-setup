 ...
 *
 * Extracted from codesoda/subagents#2 (test-only follow-up to #164) and
...
describe.skip("PR #164 nested agents through the real print-mode boundary (ponytail skip: real pi-mono e2e", () => {
  ...
  it("strips nested tools at the depth cap instead of injecting always-failing ones", async () => {
    // Default maxSubagentDepth is 4: main(0) → level_one(1) → level_two(2)
    // → level_three(3) → level_four(4). #164 injects nested tools only while
    // depth < max, so the agent at the cap
    ...
    ({ run, cwd } = await runWithAgents(
      {
        ...
        level_three: "allowed_subagents: level_four\n",
        level_four: "",
      },
      (ctx) => {
        const route = userPrompt(ctx);
        observed.set(route, tools(ctx));
        if (route === "level_four-child") {
        ...
        if (route === "level_three-child") {
          if (toolResults(ctx, "Agent").length === 0) {
            return agentCall({
              subagent_type: "level_four",
              description: "allowed level",
              prompt: "level_four-child",
              run_in_background: false,
            });
          }
          return lastToolResult(ctx, "Agent");
    ...
    expect(nestedToolsIn(observed.get("level_four-child"))).toEqual([]);
    expect(observed.has("level_five-child")).toBe(false);
  });
