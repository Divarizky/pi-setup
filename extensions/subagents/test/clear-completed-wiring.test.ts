...
async function spawnCompletedBackgroundAgent(tools: Map<string, any>): Promise<string> {
  ...
  const spawn = await tools.get("Agent").execute(
    "tc-spawn",
    { prompt: "go", description: "Review monero_en.rs in depth", subagent_type: "general", run_in_background: true },
    undefined,
    undefined,
    ctx(),
  );
...
describe("issue #108: unread completed background agents survive session events", () => {
  ...
  beforeEach(() => {
    // Hermetic cwd + global dir keep session_start from touching the developer's
    // filesystem and isolate the clearCompleted path.
