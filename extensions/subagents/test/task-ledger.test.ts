...
test("orchestration coordinator handles each event once", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "task-ledger-coordinator-"))
  try {
    const ledger = new TaskLedger(root)
    const handled: string[] = []
    const coordinator = new OrchestrationCoordinator(ledger, async (event) => { handled.push(event.eventId) })
    const event = {
