...
describe("createWorkflowHost — spawn mapping", () => {
  ...
  it("rejects an unknown agent type through the same dispatch as the Agent tool", async () => {
    const stub = stubManager();
    const host = createWorkflowHost({ pi: {} as any, ctx: ctx(), manager: stub.manager });

    const rejected = await host.spawnAgent(request({ agentType: "no-such-agent" }));
...
describe("--subagents-workflow-file", () => {
  ...
  it("registers the fleet row as soon as a run starts, not when it settles", async () => {
    ...
    context.ui.setFooter.mockClear();
    ...
    const footers = context.ui.setFooter.mock.calls.map((call: any[]) => call[0]);
    // FleetList renders as a footer-chain composite (fleet rows + custom footer).
    expect(
      footers.filter((f: unknown) => typeof f === "function"),
...
describe("collisions with another extension", () => {
  ...
  it("stays quiet when the registered tool is our own", async () => {
    ...
    booted.pi.getAllTools.mockReturnValue([
      { name: "SubagentWorkflow", description: ours.description, sourceInfo: { source: "subagents" } },
    ]);
    const context = uiContext();
