...
test("retry policy only allows failed jobs within the attempt limit", () => {
  assert.equal(canRetry({ status: "done", metrics: { restartCount: 0 } }), false)
  assert.equal(canRetry({ status: "error", metrics: { restartCount: 0 } }), true)
  assert.equal(canRetry({ status: "error", metrics: { restartCount: 3 } }), false)
})
