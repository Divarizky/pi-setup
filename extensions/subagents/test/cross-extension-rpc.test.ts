...
describe("cross-extension RPC", () => {
  ...
  beforeEach(() => {
    ...
    deps = {
      ...
      getCtx: () => ctx,
      manager,
      resolveSpawnType: (type) => ({ ok: true, type: String(type) }),
    };
  });
  ...
  describe("spawn RPC", () => {
    it("returns agent id on success", async () => {
      ...
      events.emit("subagents:rpc:spawn", {
        requestId: "req-s1", type: "general", prompt: "do stuff",
      });

      await vi.waitFor(() => expect(reply).toHaveBeenCalled());
      expect(reply).toHaveBeenCalledWith({ success: true, data: { id: "agent-42" } });
      expect(manager.spawn).toHaveBeenCalledWith(
        deps.pi, ctx, "general", "do stuff", {},
      );
    });
    ...
    it("returns error when no active session", async () => {
      ...
      events.emit("subagents:rpc:spawn", {
        requestId: "req-s3", type: "general", prompt: "x",
      });
    ...
    it("rejects an unknown type before calling manager.spawn", async () => {
      deps.resolveSpawnType = () => ({
        ok: false,
        message: 'Unknown or disabled agent type: "bad-type". Available: general.',
      });
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("subagents:rpc:spawn:reply:req-invalid", reply);
      events.emit("subagents:rpc:spawn", {
        requestId: "req-invalid", type: "bad-type", prompt: "x",
      });

      await vi.waitFor(() => expect(reply).toHaveBeenCalled());
      expect(reply).toHaveBeenCalledWith({
        success: false,
        error: 'Unknown or disabled agent type: "bad-type". Available: general.',
      });
      expect(manager.spawn).not.toHaveBeenCalled();
    });
    ...
    it("returns error when the agent fails to start after spawn returns", async () => {
      ...
      events.emit("subagents:rpc:spawn", {
        requestId: "req-s4b", type: "general", prompt: "x",
    ...
    it("scopes replies — other requestIds do not receive it", async () => {
      ...
      events.emit("subagents:rpc:spawn", {
        requestId: "req-s5", type: "general", prompt: "x",
      });
    ...
    it("unsub stops responding to spawns", async () => {
      ...
      events.emit("subagents:rpc:spawn", {
        requestId: "req-s6", type: "general", prompt: "x",
      });
  ...
  describe("concurrent requests", () => {
    it("handles multiple simultaneous spawn requests independently", async () => {
      ...
      events.emit("subagents:rpc:spawn", { requestId: "req-b", type: "general", prompt: "second" });
  ...
  describe("spawn RPC model override", () => {
    ...
    it("resolves a string model to a Model instance before manager.spawn", async () => {
      ...
      events.emit("subagents:rpc:spawn", {
        requestId: "req-m1", type: "general", prompt: "x",
    ...
    it("passes a Model object through unchanged", async () => {
      ...
      events.emit("subagents:rpc:spawn", {
        requestId: "req-m2", type: "general", prompt: "x",
    ...
    it("surfaces a clear error when the model string can't be resolved", async () => {
      ...
      events.emit("subagents:rpc:spawn", {
        requestId: "req-m3", type: "general", prompt: "x",
    ...
    it("treats an explicit null model as no override at all", async () => {
      ...
      events.emit("subagents:rpc:spawn", {
        requestId: "req-m5", type: "general", prompt: "x",
        options: { model: null },
      });
      ...
      expect(reply).toHaveBeenCalledWith({ success: true, data: { id: "agent-42" } });
      expect(manager.spawn).toHaveBeenCalledWith(
        deps.pi, ctx, "general", "x", { model: null },
      );
    });
    ...
    it("errors when ctx has no modelRegistry but a string model is given", async () => {
      ...
      events.emit("subagents:rpc:spawn", {
        requestId: "req-m4", type: "general", prompt: "x",
        options: { model: "openai-codex/gpt-5.5" },
      });
