...
describe("inline glyph mapping", () => {
  ...
  it("says a replayed agent came from the resume journal", () => {
    const lines = card({
      progress: [
        agentEntry({ index: 0, label: "audit", state: "done", cached: true, agentType: "general" }),
      ],
    });
    ...
    expect(row.indexOf("from resume journal")).toBeLessThan(row.indexOf("general"));
  });

  it("leaves an agent that actually ran unannotated", () => {
    const lines = card({
      progress: [agentEntry({ index: 0, label: "audit", state: "done", agentType: "general" })],
    });
