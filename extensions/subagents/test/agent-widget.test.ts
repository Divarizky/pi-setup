...
describe("AgentWidget", () => {
  ...
  // Also covers manager-spawned agents (isBackground=true, no `invocation`
...
describe("AgentWidget cost display", () => {
  ...
  it("keeps the cost visible after the agent finishes", () => {
    ...
    const finished = {
      id: "a1", type: "general", description: "done agent", status: "completed",
  ...
  it("shows stats for an agent nobody is tracking live", () => {
    // A manager-spawned agent has no activity entry — it enters through the manager
    // directly — and used to render with no tokens and no cost at all.
    const running = {
      id: "sched", type: "general", description: "scheduled agent", status: "running",
  ...
  it("defaults to hiding it", () => {
    const agent = {
      id: "a1", type: "general", description: "d", status: "running",
