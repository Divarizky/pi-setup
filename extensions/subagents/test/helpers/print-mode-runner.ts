 ...
 * subagents extension through REAL end-to-end subagent runs.
 *
 * WHY THIS EXISTS
 * ---------------
 ...
 * subagents extension loaded, drives a real assistant turn that calls the
 ...
 *
 * It is the subagents analogue of pi-chonky-step's `src/agent.ts` headless
 ...
 * background subagents complete (via the `Symbol.for("subagents:manager")`
...
/** Path to the subagents extension entrypoint (repo `src/index.ts`). */
 ...
 * to "general"; everything else is passed straight through as tool args.
 */
export function agentCall(
  ...
  return fauxToolCall("Agent", { subagent_type: "general", ...args }, opts);
}
