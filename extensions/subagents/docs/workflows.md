...
Script: /var/folders/xy/…/subagents-501/Users-me-project/<session>/tasks/wf_9f3ab21c04de.workflow.js
...
| `agentType` | string | Which agent definition to use. Defaults to `general`; built-ins are `explore`, `build`, and `general`, plus your custom agents |
...
| `gate` | string | A single shell command run after the agent finishes (no chaining: no `&&`, `\|\|`, `;`, pipes, backticks, or `$()`); a non-zero exit fails the agent and its output becomes the error. Max 1024 chars, output truncated at 8KB |
...
Any other key is rejected **by name** at the call. An `agentType` that names no enabled agent is rejected with an error; matching is case-insensitive, and disabled agents do not count.
...
| What | Where |
|---|---|
| An inline script, as run | `<tmp>/subagents-<uid>/<encoded-cwd>/<session>/tasks/<run id>.workflow.js` |
...
Above 25 agents, or 1.5M tokens actual or projected, the card adds `⚠ Large workflow · /agents → Workflows to stop`.
...
`workflowsEnabled` is **on**; leaving it unset means *auto*, which is on unless another extension already offers a `Workflow` or `SubagentWorkflow` tool, in which case this one stands down for the session. Setting it explicitly pins it. See [Settings](../README.md#settings).
...
**An agent type was rejected.**
Check the `agentType` spelling against `/agents`; matching is case-insensitive, and disabled or unknown agents are rejected before the child starts.
