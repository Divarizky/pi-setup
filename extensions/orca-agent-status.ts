...
const CONFIGURED_HOOK_PATH: string = "/hook/pi";
...
async function postOnce(
  ...
  const body = JSON.stringify({
    ...
    payload: {
      ...
      ...(ompRuntime ? metadata : getPersistedSessionMetadata()),
      ...extra,
    },
  });
  const controller =
  ...
  try {
    ...
    const child = spawn(
      ...
      [
        ...
        "Content-Type: application/json",
        "-H",
        `X-Orca-Agent-Hook-Token: ${windowsToken}`,
        "--data-binary",
        "@-",
        windowsUrl,
      ],
...
export default function (pi: ExtensionAPI): void {
  ...
  function checkPendingAgentEnd(): void {
    ...
    pendingAgentEndCheck = setTimeout(
      checkPendingAgentEnd,
      agentEndIdleRecheckMs,
    );
    if (typeof pendingAgentEndCheck.unref === "function")
      pendingAgentEndCheck.unref();
    agentEndIdleRecheckMs = Math.min(
      agentEndIdleRecheckMs * 2,
      AGENT_END_IDLE_RECHECK_MAX_MS,
    );
  }
