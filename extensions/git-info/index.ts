...
export default function gitInfo(pi: ExtensionAPI) {
  ...
  // Keep only immutable data in background work. ExtensionContext becomes stale
  // when Pi replaces or reloads a session.
  ...
  pi.on("session_start", async (_event, ctx) => {
    // Read cwd while this event's context is active; never capture ctx in a
    // background effect that may outlive this session.
    const cwd = ctx.cwd;
    generation += 1;
    currentCwd = cwd;
    queriedPrBranch = null;
  ...
  pi.registerCommand("pr", {
    ...
    handler: async (_args, ctx) => {
      const cwd = ctx.cwd;
      await runEffect(getRuntime(), refresh(cwd, true), {
        signal: ctx.signal,
