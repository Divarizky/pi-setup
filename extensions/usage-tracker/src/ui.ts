...
function limitLines(theme: Theme, limit: UsageLimit, width: number, now: Date): string[] {
  ...
  const right = theme.fg(limit.usedPercent >= 90 ? "warning" : "text", `${used} terpakai`);
  return [
...
function periodLine(theme: Theme, label: string, usage: ProviderUsage["today"]): string {
...
function providerRows(theme: Theme, usage: ProviderUsage, width: number, now: Date): string[] {
  ...
  if (usage.limits?.length) {
    ...
    rows.push(...usage.limits.flatMap((limit) => limitLines(theme, limit, inner, now)));
  } else {
    rows.push(
      line(
        theme,
