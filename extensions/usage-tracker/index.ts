...
async function loadUsage(ctx: ExtensionContext): Promise<UsageTrackerViewData> {
  ...
  try {
    ...
    const providers = fetched.filter((provider) => provider.limits && provider.limits.length > 0);
...
export default function usageTrackerExtension(pi: ExtensionAPI) {
  pi.registerCommand("usage-tracker", {
    ...
    handler: async (_args, ctx) => {
      ...
      ctx.ui.setWidget(
        loadingWidgetKey,
        (_tui, theme) => new Text(theme.fg("dim", "Mengambil usage provider secara real-time…"), 1, 0),
