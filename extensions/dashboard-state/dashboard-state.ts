...
export interface MemoryStatusState {
  ok: boolean;
  shortPath: string;
}

export function emptyMemoryStatusState(): MemoryStatusState {
  return {
    ok: false,
    shortPath: "",
  };
...
export function isMemoryStatusState(
  value: unknown,
): value is MemoryStatusState {
  if (!isRecord(value)) return false;

  return typeof value.ok === "boolean" && typeof value.shortPath === "string";
}
