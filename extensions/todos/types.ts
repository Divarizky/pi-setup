/**
 * Domain types + TypeBox parameter schema for the todos extension.
 *
 * Tool name "todo" and command name "todos" are pinned: the branch-replay
 * persistence key filters `toolResult.toolName === "todo"`, so renaming
 * breaks session replay. Keep them stable.
 */
...
// ---------------------------------------------------------------------------
// User-facing strings (English fallbacks; UI chrome routes through locale.ts)
// ---------------------------------------------------------------------------
 ...
 * this shape under `details`; `state/store.ts` reads the latest one from the
 * branch to reconstruct module state. Field order and names are pinned by
