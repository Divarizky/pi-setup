...
test("create: blockedBy validates existence and non-deleted", () => {
  ...
  const missing = applyTaskMutation(withTask, "create", {
    subject: "B",
    blockedBy: [99],
  });
  ...
  const onDeleted = applyTaskMutation(deleted, "create", {
    subject: "B",
    blockedBy: [1],
  });
...
test("update: status transition + changed flag", () => {
  ...
  const r = applyTaskMutation(state, "update", {
    id: 1,
    status: "in_progress",
  ...
  const noop = applyTaskMutation(r.state, "update", {
    id: 1,
    status: "in_progress",
  });
...
test("update: id required / not found / no mutable field", () => {
  let state = applyTaskMutation(fresh(), "create", { subject: "A" }).state;
  assert.equal(
    applyTaskMutation(state, "update", { status: "completed" }).op.kind,
    "error",
  );
  assert.equal(
    applyTaskMutation(state, "update", { id: 99, status: "completed" }).op.kind,
    "error",
  );
...
test("update: metadata merge + null delete", () => {
  let state = applyTaskMutation(fresh(), "create", {
    subject: "A",
    metadata: { k1: "v1", k2: "v2" },
  }).state;
  state = applyTaskMutation(state, "update", {
    id: 1,
    metadata: { k2: null, k3: "v3" },
  }).state;
...
test("update: addBlockedBy self-block, missing dep, cycle", () => {
  ...
  const missing = applyTaskMutation(state, "update", {
    id: 1,
    addBlockedBy: [42],
  });
  assert.equal(missing.op.kind, "error");
  ...
  const cycle = applyTaskMutation(state, "update", {
    id: 2,
    addBlockedBy: [1],
  });
...
test("delete: tombstones; already-deleted rejected", () => {
  ...
  assert.equal(
    applyTaskMutation(r.state, "delete", { id: 1 }).op.kind,
    "error",
  );
});
...
test("selectors: counts, groups, active, showIds, layout", () => {
  let state = applyTaskMutation(fresh(), "create", { subject: "A" }).state;
  state = applyTaskMutation(state, "create", {
    subject: "B",
    blockedBy: [1],
  }).state;
  state = applyTaskMutation(state, "update", {
  ...
  assert.deepEqual(selectTodoCounts(state), {
    total: 3,
    pending: 1,
    inProgress: 1,
    completed: 1,
  ...
  assert.deepEqual(
    selectTasksByStatus(state).inProgress.map((t) => t.id),
    [1],
  );
  ...
  assert.deepEqual(
    replayFromBranch(
      mockCtx([
        {
          type: "message",
          message: { role: "toolResult", toolName: "todo", details: "junk" },
        },
      ]),
    ),
    EMPTY_STATE,
