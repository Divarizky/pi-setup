...
import { formatOverlayTaskLine, ICON_FILLED, STATUS_COLOR } from "./format.ts";

const WIDGET_KEY = "todos";
const WIDGET_HORIZONTAL_PADDING = " ";
...
export class TodosOverlay {
	...
	private selectOverlayTasks(snapshot: ReturnType<TodosOverlay["getSnapshot"]>) {
		return snapshot.tasks.filter((task) => task.status !== "deleted" && !this.shouldHideCompletedTask(task));
	}

	private shouldHideCompletedTask(task: ReturnType<TodosOverlay["getSnapshot"]>["tasks"][number]): boolean {
	...
	private renderWidget(theme: Theme, width: number): string[] {
		...
		// Unlike array-form widgets, this factory-form widget does not receive
		// Text's default one-column horizontal padding. Match the Subagents
		// widget on both sides so headings and tree branches share the same
		// starting column and the same effective content width.
		const truncate = (line: string): string =>
			line.length === 0
				? line
				: truncateToWidth(
					`${WIDGET_HORIZONTAL_PADDING}${line}${WIDGET_HORIZONTAL_PADDING}`,
					width,
					"…",
				);
		...
		if (this.collapsed) {
			const hint =
				this.collapseKey === COLLAPSE_KEY_OFF
					...
					: t("overlay.expandHint", OVERLAY_EXPAND_HINT).replace("{key}", this.collapseKey);
		...
		for (const task of layout.visible) {
			lines.push(truncate(`${theme.fg("dim", "├─")} ${formatOverlayTaskLine(task, theme, showIds, allDone)}`));
		}
