 ...
 */
export function registerLangCommand(pi: ExtensionAPI, onLocaleChanged?: () => void): void {
	pi.registerCommand("lang", {
		description: "Switch UI language (en | id)",
		handler: async (args, ctx) => {
			const lang = args.trim().toLowerCase();
			if (!isSupportedLocale(lang)) {
				ctx.ui.notify(t("lang.usage", "Usage: /lang <en|id>. Supported: en, id"), "error");
				return;
			}
			setLocale(lang);
			onLocaleChanged?.();
			ctx.ui.notify(t("lang.changed", `Language: ${lang}`).replace("{0}", lang), "info");
		},
	});
}
