/**
 * Pure selectors over TaskState — no module state, no side effects.
 */
...
/** Any visible task carries a `blockedBy` reference → show `#id` prefixes. */
...
/** Resolve a task's subject by id for renderCall's accent label. */
...
/**
 * Overlay layout decision: "drop completed first, then truncate non-completed
 * tail". `budget` is the body-slot count (caller passes
 * `getMaxWidgetLines() - 1`); on overflow one slot is reserved for the
 * summary row.
 */
...
/** Any visible task pending/in_progress → heading icon/color choice. */
