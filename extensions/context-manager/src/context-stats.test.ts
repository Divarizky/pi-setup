...
test("formatContextStats reports savings and inspect usage", () => {
  ...
  assert.match(output, /│  ├─ Pemrosesan: 10 karakter \(1 baris\) → 5 karakter \(1 baris\)/);
  assert.match(output, /│  └─ Penghematan: 5 karakter, 0 baris \(50,0% hemat\)/);
  ...
  assert.match(output, /│  └─ Hasil: 1 output \| 3 karakter \| 0 token dihemat/);
  assert.match(output, /├─ Aktivitas/);
  assert.match(output, /│  └─ Operasi: 1 output diringkas \| 2 kali inspect \| 1 kali output cache/);
